 ...
 */
export function decryptCallback(privateKeyDer: Buffer, encryptedB64: string) {
 ...
 * @returns {string}
 */
export function buildAuthorizeUrl(publicKey: string, redirectUri: string, keyName: string | undefined) {
 ...
 * Stored in the OpenCursor data dir so re-auth reuses the same name.
 */
export function getKeyName() {
  ...
  const machineId = crypto
    ...
    .digest("hex")
    .slice(0, 8);
  return `opencursor-xmd-${machineId}`;
}
