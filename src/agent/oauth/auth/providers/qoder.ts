 ...
 * oauth block (see the Qoder entries in protocols/providers/index.ts), which
 * carries the region's login/deviceToken/userInfo URLs. The device flow is
 * identical across regions — only the hosts differ.
 */
export function createQoderProvider(config: AuthConfig): AuthProvider {
  return {
    config,
    ...
    requestDeviceCode: async (cfg) => {
      const { QoderService } = await import("../services/qoder.js");
    ...
    pollToken: async (cfg, deviceCode, codeVerifier, extraData) => {
      ...
      try {
        ...
        return {
          ok: false,
          data: { error: "poll_failed", error_description: authErrorMessage(err) },
        };
      }
