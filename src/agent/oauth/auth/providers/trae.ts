...
// Per-login device context. No IDE access in OpenCursor, so use stable defaults.
...
async function fetchTraeLoginGuidance(loginTraceId: string) {
  ...
  for (const url of TRAE_CONFIG.loginGuidanceUrls) {
    ...
    } catch (e) { lastErr = `${url} ${authErrorMessage(e)}`; }
  }
...
function parseTraeCallback(raw: unknown) {
...
function traeScopeForRegion(aiRegion: string) {
