...
import type { AuthConfig, AuthPayload, AuthLogger } from "../types.js";
import { authFetch as fetch } from "../network.js";
import { PROVIDERS, PROVIDER_OAUTH } from "../../protocols/config/providers.js";
import { OAUTH_ENDPOINTS, GITHUB_COPILOT, buildKimiHeaders } from "../../protocols/config/appConstants.js";
import { authFetch as proxyAwareFetch } from "../network.js";
import { dedupRefresh } from "./dedup.js";
import { buildExternalIdpRefreshParams } from "../kiroExternalIdp.js";

import xai from "../providers/xai.js";
export async function refreshXaiToken(refreshToken: string) {
  const config = await xai.prepareConfig!(xai.config);
  const response = await fetch(config.tokenUrl, { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded", Accept: "application/json" }, body: new URLSearchParams({ grant_type: "refresh_token", client_id: config.clientId, refresh_token: refreshToken }) });
  if (!response.ok) return { error: response.status === 400 || response.status === 401 ? "invalid_grant" : "refresh_failed" };
  const data = await response.json();
  return { accessToken: data.access_token, refreshToken: data.refresh_token || refreshToken, expiresIn: data.expires_in, idToken: data.id_token };
}
...
interface RefreshProfile {
  ...
  includeClientSecret?: boolean | ((config: AuthConfig) => boolean);
  url?: () => string;
  dedupKey?: string;
  extraHeaders?: (credentials: AuthPayload, config: AuthConfig) => Record<string, string>;
  parse?: (tokens: AuthPayload) => AuthPayload;
}
...
function resolveRefreshUrl(provider: string, config: AuthConfig, profile: RefreshProfile) {
  if (profile?.url) {
...
function buildRefreshBody(profile: RefreshProfile, config: AuthConfig, refreshToken: string) {
...
export async function refreshAccessToken(provider: string, refreshToken: string, credentials: AuthPayload, log?: AuthLogger) {
...
export async function refreshKimiToken(refreshToken: string, credentials: AuthPayload, log?: AuthLogger) {
  return refreshAccessToken("kimi", refreshToken, credentials, log);
}

export async function refreshClineToken(refreshToken: string, log?: AuthLogger) {
  ...
  return dedupRefresh("cline", refreshToken, async () => {
    try {
      ...
      log?.error?.("TOKEN_REFRESH", `Error refreshing Cline token: ${authErrorMessage(error)}`);
      return null;
    }
  }, log);
}
...
export async function refreshClaudeOAuthToken(refreshToken: string, log?: AuthLogger) {
  return refreshAccessToken("claude", refreshToken, {}, log);
}

export async function refreshGoogleToken(refreshToken: string, clientId: string, clientSecret: string, log?: AuthLogger) {
  if (!refreshToken) return null;
  ...
  try {
    ...
    log?.error?.("TOKEN_REFRESH", `Network error refreshing Google token: ${authErrorMessage(error)}`);
    return null;
  }
  }, log);
}

export function classifyOAuthRefreshError(errorText: string = "", status: number = 0) {
  let parsed = null;
  try {
...
export async function refreshCodexToken(refreshToken: string, log?: AuthLogger) {
  if (!refreshToken) return null;
  return dedupRefresh("codex", refreshToken, async () => {
    try {
      ...
      log?.error?.("TOKEN_REFRESH", `Network error refreshing Codex token: ${authErrorMessage(error)}`);
      return null;
    }
  }, log);
}

async function resolveKiroProfileArnPatch(providerSpecificData: AuthPayload, accessToken: string, refreshedArn: string | null | undefined) {
  if (providerSpecificData?.profileArn) return {};
  let profileArn = refreshedArn?.trim?.() || null;
  if (!profileArn) {
    const { fetchKiroProfileArn } = await import("../providerHelpers.js");
    profileArn = await fetchKiroProfileArn(accessToken);
  }
  return profileArn ? { providerSpecificData: { profileArn } } : {};
}

export async function refreshKiroToken(refreshToken: string, providerSpecificData: AuthPayload, log?: AuthLogger, proxyOptions: AuthPayload | null = null) {
  if (!refreshToken) return null;
  ...
  if (authMethod === "external_idp") {
    ...
    try {
      ...
      log?.warn?.("TOKEN_REFRESH", `Invalid Kiro external_idp refresh config: ${authErrorMessage(error)}`);
      return null;
    }
...
export async function refreshIflowToken(refreshToken: string, log?: AuthLogger) {
  return refreshAccessToken("iflow", refreshToken, {}, log);
}

// GitHub: optional client_secret. Delegate to refreshAccessToken("github", ...).
export async function refreshGitHubToken(refreshToken: string, log?: AuthLogger) {
  return refreshAccessToken("github", refreshToken, {}, log);
}

export async function refreshCopilotToken(githubAccessToken: string, log?: AuthLogger) {
...
export async function refreshCodebuddyToken(refreshToken: string, log?: AuthLogger) {
  if (!refreshToken) return null;
...
export async function refreshCodebuddyIntlToken(refreshToken: string, log?: AuthLogger) {
  if (!refreshToken) return null;
...
export async function refreshTraeToken(refreshToken: string, credentials: AuthPayload, log?: AuthLogger) {
  if (!refreshToken) return null;
  ...
  return dedupRefresh("trae", refreshToken, async () => {
    try {
      ...
      log?.error?.("TOKEN_REFRESH", `Error refreshing Trae token: ${authErrorMessage(error)}`);
      return null;
    }
  }, log);
}
...
export async function refreshWindsurfToken(credentials: AuthPayload, log?: AuthLogger) {
  log?.info?.(
    "TOKEN_REFRESH",
