...
export class QoderService {
   ...
   *   { status: "pending" as const }       — keep polling
   *   { status: "ok" as const, token, ... } — user authorized, tokens captured
   *   throws Error                 — terminal failure
   *
   * Upstream returns 202/404 while waiting; 200 with a JSON body when done.
   */
  async pollDeviceToken({ nonce, codeVerifier }: { nonce: string; codeVerifier: string }) {
    ...
    try {
      ...
      throw new Error(`Qoder device token poll: invalid JSON response (${authErrorMessage(err)})`);
    }
   ...
   */
  static parseExpiry(expiresAt: unknown, expiresInSeconds: unknown) {
