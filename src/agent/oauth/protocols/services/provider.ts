...
const OPENAI_COMPATIBLE_DEFAULTS: ProtocolRecord = {
    baseUrl: OPENAI_COMPAT_BASE,
};
const ANTHROPIC_COMPATIBLE_PREFIX = "anthropic-compatible-";
const ANTHROPIC_COMPATIBLE_DEFAULTS: ProtocolRecord = {
...
export function resolveOpenAICompatibleApiType(provider: string, credentials: ProtocolValue = null) {
    const stored = credentials?.providerSpecificData?.apiType;
    if (stored === "chat" || stored === "responses")
        return stored;
    return typeof provider === "string" && provider.includes("responses") ? "responses" : "chat";
}
// Detect request format from body structure
export function detectFormat(body: ProtocolRecord) {
    ...
    if (body.stream_options || // OpenAI streaming options
    ...
    if (body.messages && Array.isArray(body.messages)) {
        ...
        if (firstMsg?.content && Array.isArray(firstMsg.content)) {
            ...
            if (firstContent?.type === "text" && !body.model?.includes("/")) {
                ...
                const hasClaudeImage = firstMsg.content.some((c: ProtocolValue) => c.type === "image" && c.source?.type === "base64");
                const hasOpenAIImage = firstMsg.content.some((c: ProtocolValue) => c.type === "image_url" && c.image_url?.url);
                if (hasClaudeImage)
                    return "claude";
                if (hasOpenAIImage)
                    return "openai";
                // If still unclear, check for tool format
                const hasClaudeTool = firstMsg.content.some((c: ProtocolValue) => c.type === "tool_use" || c.type === "tool_result");
                if (hasClaudeTool)
                    return "claude";
            }
        }
...
function getProviderConfig(provider: string, credentials: ProtocolValue = null) {
...
export function getTargetFormat(provider: string, credentials: ProtocolValue = null) {
    if (isOpenAICompatible(provider)) {
...
export function resolveTransport(provider: string, sourceFormat: string) {
    ...
    if (!Array.isArray(transports) || !transports.length)
        return null;
    return transports.find((t: ProtocolValue) => t.format === sourceFormat) || null;
}
// Check if last message is from user
export function isLastMessageFromUser(body: ProtocolRecord) {
...
export function hasThinkingConfig(body: ProtocolRecord) {
...
export function normalizeThinkingConfig(body: ProtocolRecord) {
