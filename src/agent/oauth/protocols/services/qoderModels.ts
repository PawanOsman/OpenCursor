...
import { QODER_IDE_VERSION, QODER_CLIENT_TYPE, qoderRegionOf, qoderJobTokenExchangeUrl, qoderUserInfoUrl, qoderInferenceBase, } from "../shared/qoder/constants.js";
...
export function isQoderPat(token: ProtocolValue) {
 ...
 */
async function exchangeJobToken(pat: ProtocolValue, proxyOptions: ProtocolValue = null, signal: ProtocolValue = null, region = "intl") {
    const res = await proxyAwareFetch(qoderJobTokenExchangeUrl(region), {
        method: "POST",
        headers: {
    ...
    const data: ProtocolValue = await res.json();
    if (!data.token)
 ...
 */
async function fetchUserIdForJobToken(jobToken: ProtocolValue, proxyOptions: ProtocolValue = null, signal: ProtocolValue = null, region = "intl") {
    try {
        const res = await proxyAwareFetch(qoderUserInfoUrl(region), {
            method: "GET",
            headers: {
        ...
        if (!res.ok)
            return "";
        const data: ProtocolValue = await res.json().catch(() => ({}));
 ...
 */
async function resolvePatCredential(pat: ProtocolValue, proxyOptions: ProtocolValue = null, signal: ProtocolValue = null, region = "intl") {
    ...
    const resolved: ProtocolRecord = { accessToken: jobToken, userId, expiresAt };
 ...
 */
export async function resolveQoderCredentials(credentials: ProtocolCredentials, proxyOptions: ProtocolValue = null, signal: ProtocolValue = null, region: string) {
 ...
 */
function cacheKey(credentials: ProtocolCredentials) {
 ...
 */
function cosyCredsFromConnection(credentials: ProtocolCredentials) {
    const psd = credentials?.providerSpecificData || {};
    return {
        userId: psd.userId,
 ...
 */
async function fetchQoderCatalogRaw(credentials: ProtocolCredentials, signal: ProtocolValue, proxyOptions: ProtocolValue = null, region = "intl") {
    ...
    let timer: ProtocolValue = null;
    let abortListener: ProtocolValue = null;
    let response: ProtocolValue;
    try {
        ...
        response = await proxyAwareFetch(modelListUrl, {
            method: "GET",
            headers,
    ...
    if (!response.ok)
        return null;
    const body: ProtocolValue = await response.json().catch(() => null);
    if (!body || !Array.isArray(body.chat))
        return null;
    const models: ProtocolValue[] = [];
    ...
    for (const entry of body.chat) {
        ...
        const ctx = Number(entry.max_input_tokens) || 131072;
        models.push({
            id: key,
            name: `${display}`,
            contextLength: ctx,
 ...
 */
export async function getQoderModelConfig(credentials: ProtocolCredentials, modelKey: ProtocolValue, options: ProtocolRecord = {}) {
 ...
 */
export async function resolveQoderModels(credentials: ProtocolCredentials, options: ProtocolRecord = {}) {
    const region = options.region || qoderRegionOf(credentials?.provider);
    let resolved: ProtocolValue;
    try {
 ...
 */
export function routableQoderModels(catalog: ProtocolValue) {
    if (!catalog)
        return [];
...
export function invalidateQoderCatalog(credentials: ProtocolCredentials) {
    if (!credentials)
        return;
