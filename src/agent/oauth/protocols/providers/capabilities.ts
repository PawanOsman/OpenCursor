 ...
 */
export const DEFAULT_CAPABILITIES: ProtocolRecord = {
    // input modalities
...
const SERVICE_KIND_CAPABILITIES: ProtocolRecord = {
...
export function capabilitiesFromServiceKind(kind: ProtocolValue) {
 ...
 */
export const MODEL_CAPABILITIES: ProtocolRecord = {
...
const KIRO_GPT_5_6_CAPABILITIES: ProtocolRecord = { vision: true, reasoning: true, search: true, thinkingFormat: "openai", contextWindow: 272000, maxOutput: 128000 };
...
const CODEX_GPT_56_SOL_CAPS: ProtocolRecord = { vision: true, reasoning: true, search: true, thinkingFormat: "openai", contextWindow: 372000, maxOutput: 128000 };
const CODEX_GPT_56_DEFAULT_CAPS: ProtocolRecord = { vision: true, reasoning: true, search: true, thinkingFormat: "openai", contextWindow: 272000, maxOutput: 128000 };
/**
 * Provider-specific capability overrides. Keyed by provider alias/id.
 */
export const PROVIDER_CAPABILITIES: ProtocolRecord = {
 ...
 */
export const PATTERN_CAPABILITIES: ProtocolValue[] = [
 ...
 */
export function aggregateComboCapabilities(comboModels: ProtocolValue, comboLookup: ProtocolValue = null, _depth = 0) {
    if (!comboModels?.length || _depth > 6)
        return null;
    const allCaps = comboModels.map((fullId: ProtocolValue) => {
    ...
    return {
        vision: allCaps.some((c: ProtocolValue) => c.vision),
        pdf: allCaps.some((c: ProtocolValue) => c.pdf),
        audioInput: allCaps.some((c: ProtocolValue) => c.audioInput),
        videoInput: allCaps.some((c: ProtocolValue) => c.videoInput),
        imageOutput: allCaps.some((c: ProtocolValue) => c.imageOutput),
        audioOutput: allCaps.some((c: ProtocolValue) => c.audioOutput),
        search: allCaps.some((c: ProtocolValue) => c.search),
        tools: allCaps.every((c: ProtocolValue) => c.tools),
        reasoning: first.reasoning,
        thinkingFormat: first.thinkingFormat,
        thinkingCanDisable: first.thinkingCanDisable,
        thinkingRange: first.thinkingRange,
        contextWindow: Math.min(...allCaps.map((c: ProtocolValue) => c.contextWindow)),
        maxOutput: Math.max(...allCaps.map((c: ProtocolValue) => c.maxOutput)),
    };
}
/**
 ...
 */
const MODALITY_KEYS: string[] = ["vision", "pdf", "audioInput", "videoInput"];
interface CatalogSource {
    getModalities(provider: string | null, model: string): Record<string, boolean> | null;
    getLimits(provider: string | null, model: string): { contextWindow: number; maxOutput: number } | null;
}
// OpenCursor has a single extension-host module graph; no global catalog slot is needed.
...
function getCatalogSource(): CatalogSource | null {
...
function refine(base: ProtocolRecord | null, provider: string | null, model: string) {
    const result: ProtocolRecord = { ...DEFAULT_CAPABILITIES, ...base };
    const source = getCatalogSource();
    if (source) {
        const modalities = source.getModalities(provider, model);
        if (modalities) {
            for (const key of MODALITY_KEYS) {
                if (modalities[key] === true)
                    result[key] = true;
            }
        }
...
function isCommandCodeTextOnly(model: string) {
    const key = String(model || "").toLowerCase();
    if (COMMANDCODE_TEXT_ONLY.has(key))
        return true;
...
export function getCapabilitiesForModel(provider: string | null, model: string) {
    if (!model)
