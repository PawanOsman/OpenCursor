 ...
 */
export const MODEL_PRICING: ProtocolRecord = {
    // === Anthropic / Claude ===
 ...
 */
export const PROVIDER_PRICING: ProtocolRecord = {
 ...
 */
export const PATTERN_PRICING: ProtocolValue[] = [
    // --- Codex variants ---
 ...
 */
export function matchPattern(pattern: ProtocolValue, model: string) {
    const regex = new RegExp("^" + pattern.split("*").map((s: ProtocolValue) => s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join(".*") + "$", "i");
 ...
 * @returns {string}
 */
export function formatCost(cost: ProtocolValue) {
 ...
 */
export function calculateCostFromTokens(tokens: ProtocolValue, pricing: ProtocolValue) {
    if (!tokens || !pricing)
        return 0;
    let cost = 0;
