...
const ANTHROPIC_BETA_BASE: string[] = [
...
const ANTHROPIC_BETA_HEAVY_AGENT: string[] = ["advanced-tool-use-2025-11-20", "effort-2025-11-24"];
...
export function wantsThinkingSummaries(body: ProtocolRecord) {
    return body?.thinking?.display === "summarized";
}
export function selectAnthropicBeta(model = "", body: ProtocolValue = null) {
    const flags = ANTHROPIC_BETA_BASE.filter((flag: ProtocolValue) => flag !== ANTHROPIC_BETA_REDACT_THINKING || !wantsThinkingSummaries(body));
...
// Keep this static even when OpenCursor runs on Linux: the provider profile is
...
export const ANTIGRAVITY_OAUTH_CLIENT: Record<string, string> = {
...
export const GOOGLE_OAUTH_CLIENT: Record<string, string> = {
