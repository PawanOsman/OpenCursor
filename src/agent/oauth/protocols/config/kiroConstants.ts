 ...
 *
 * Handles provider-specific request conventions:
 *
 ...
 *
 * Kiro upstream does not advertise `-agentic` model IDs; they are a OpenCursor
...
export const KIRO_CODEWHISPERER_TARGET = "AmazonCodeWhispererStreamingService.GenerateAssistantResponse";
...
export const KIRO_DEFAULT_PROFILE_ARNS: Record<string, string> = {
...
export function resolveDefaultProfileArn(authMethod: ProtocolValue) {
 ...
 * The suffix is a OpenCursor request override, not part of Kiro's upstream model id.
 */
...
export function applyKiroThinkingOverride(body: ProtocolRecord, override: ProtocolValue) {
    if (!override)
        return body;
    const next: ProtocolRecord = { ...body };
 ...
 */
export function resolveKiroThinkingBudget(body: ProtocolRecord, headers: ProtocolValue, model: string) {
    const cfg = extractThinking(body);
    if (cfg) {
        if (cfg.mode === "none")
            return null;
        if (cfg.mode === "level" && cfg.level === "disabled")
            return null;
        if (cfg.mode === "budget")
            return cfg.budget;
        if (cfg.mode === "level")
...
export function extractKiroEffortLevel(body: ProtocolRecord) {
...
function extractKiroGptEffortLevel(body: ProtocolRecord) {
    const effort = body?.output_config?.effort ??
        body?.reasoning_effort ??
        (typeof body?.reasoning === "object" ? body.reasoning?.effort : null);
    if (typeof effort !== "string")
...
export function buildKiroAdditionalModelRequestFields(body: ProtocolRecord, effortPath = "output_config") {
...
export function usesKiroNativeGptEffort(body: ProtocolRecord, model: string) {
    return resolveKiroEffortPath(model) === "reasoning"
        && extractKiroGptEffortLevel(body) !== null;
}
export function buildKiroAdditionalModelRequestFieldsForModel(body: ProtocolRecord, model: string) {
 ...
 * @returns {boolean}
 */
export function isThinkingEnabled(body: ProtocolRecord, headers: ProtocolValue, model: string) {
    return resolveKiroThinkingBudget(body, headers, model) !== null;
}
/**
 * Detect whether a model id refers to a OpenCursor synthetic agentic variant.
...
/**
 * Detect whether a model id is a OpenCursor synthetic thinking variant
...
/**
 * Resolve a OpenCursor model id to the real upstream Kiro model id, plus flags
 ...
 * Uses the provider's request shape.
 *
 * @param {number} [budget=KIRO_THINKING_BUDGET_DEFAULT]
 */
export function buildThinkingSystemPrefix(budget: number = KIRO_THINKING_BUDGET_DEFAULT) {
...
function pickHeader(headers: ProtocolValue, name: ProtocolValue) {
    if (!headers)
        return undefined;
...
function containsThinkingModeTag(body: ProtocolRecord) {
    ...
    for (const msg of messages) {
        ...
        if (typeof content === "string") {
            if (containsTagInText(content))
                return true;
        }
        else if (Array.isArray(content)) {
            for (const part of content) {
                const text = part?.text;
                if (typeof text === "string" && containsTagInText(text))
                    return true;
            }
        }
    }
...
function containsTagInText(text: ProtocolValue) {
    if (!text)
        return false;
    if (!text.includes("<thinking_mode>"))
        return false;
