...
export const GITHUB_COPILOT: ProtocolRecord = {
...
// === Antigravity enums ===
export const IDE_TYPE: Record<string, number> = {
    UNSPECIFIED: 0,
    JETSKI: 10,
    ANTIGRAVITY: 9,
    PLUGINS: 7
...
export const PLUGIN_TYPE: Record<string, number> = {
    UNSPECIFIED: 0,
    CLOUD_CODE: 1,
    GEMINI: 2
};
...
export const CLIENT_METADATA: ProtocolRecord = {
    ideType: IDE_TYPE.ANTIGRAVITY,
    platform: getPlatformEnum(),
    pluginType: PLUGIN_TYPE.GEMINI
};
// Internal anti-loop header
export const INTERNAL_REQUEST_HEADER: Record<string, string> = { name: "x-request-source", value: "local" };
...
export const ANTIGRAVITY_HEADERS: ProtocolRecord = {
    "User-Agent": ANTIGRAVITY_IDE_USER_AGENT
};
// Cloud Code Assist API endpoints differ by client ecosystem.
export const CLOUD_CODE_API: ProtocolRecord = {
    "gemini-cli": {
...
export const LOAD_CODE_ASSIST_HEADERS: ProtocolRecord = {
...
export const ANTIGRAVITY_LOAD_CODE_ASSIST_HEADERS: ProtocolRecord = {
    "Content-Type": "application/json",
    "User-Agent": ANTIGRAVITY_IDE_USER_AGENT,
};
export const LOAD_CODE_ASSIST_METADATA: ProtocolRecord = {
    ideType: IDE_TYPE.ANTIGRAVITY,
    platform: getPlatformEnum(),
    pluginType: PLUGIN_TYPE.GEMINI,
};
...
export const ANTIGRAVITY_PROMPT_REWRITES: ProtocolValue[] = [
    ...
    { from: /opencode/gi, to: (m: ProtocolValue) => (m === "OpenCode" ? "Antigravity" : m === "OPENCODE" ? "ANTIGRAVITY" : "antigravity") }
];
...
export const REFRESH_LEAD_MS = Object.fromEntries(Object.entries(PROVIDER_OAUTH).filter(([, o]: ProtocolValue) => o.refreshLeadMs).map(([id, o]: ProtocolValue) => [id, o.refreshLeadMs]));
// OAuth endpoints
export const OAUTH_ENDPOINTS: ProtocolRecord = {
...
function getAppPackageVersion(): string { return "0.2.0"; }
...
export function buildKimiHeaders(deviceId: ProtocolValue) {
    const osName = platform();
    const architecture = arch();
    let deviceModel = `${osName} ${architecture}`;
    if (osName === "darwin")
        deviceModel = `macOS ${architecture}`;
    else if (osName === "win32")
        deviceModel = `Windows ${architecture}`;
    else if (osName === "linux")
        deviceModel = `Linux ${architecture}`;
