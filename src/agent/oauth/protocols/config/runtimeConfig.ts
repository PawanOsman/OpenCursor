...
export const MEMORY_CONFIG: ProtocolRecord = {
...
function envMs(name: ProtocolValue, def: ProtocolValue) {
...
function envUrl(name: ProtocolValue, def: ProtocolValue) {
...
export const TOKEN_SAVER_HEADER = "x-opencursor-token-saver";
// Retry config for 429 responses (legacy - kept for backward compatibility)
export const RETRY_CONFIG: Record<string, number> = {
    maxAttempts: 2,
    delayMs: 2000
};
...
export const DEFAULT_RETRY_CONFIG: ProtocolRecord = {
...
export function resolveRetryEntry(entry: ProtocolValue) {
    if (entry == null)
