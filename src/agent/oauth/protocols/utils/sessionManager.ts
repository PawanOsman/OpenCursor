 ...
 * Since OpenCursor is long-running, we simulate this "per-launch" behavior by
 * storing a generated ID in memory for each connection.
 *
 * - If OpenCursor restarts, the ID changes (matching binary restart behavior).
...
const SESSION_HEADER_KEYS: string[] = ["x-session-id", "session-id", "session_id", "x-amp-thread-id"];
const CLAUDE_CODE_SESSION_RE = /_session_([a-f0-9-]+)$/;
const CLAUDE_CODE_SESSION_HEADER = "x-claude-code-session-id";
function sha16(text: ProtocolValue) {
    return crypto.createHash("sha256").update(text).digest("hex").slice(0, 16);
}
// Normalize a session id candidate (trim, length cap)
function normalizeSessionId(value: ProtocolValue) {
...
function extractClaudeCodeSession(userId: ProtocolValue) {
...
function headerValue(headers: ProtocolValue, key: ProtocolValue) {
...
function extractAntigravitySession(body: ProtocolRecord) {
    const sid = body?.request?.sessionId;
    if (sid != null && sid !== "")
        return normalizeSessionId(String(sid));
    const m = typeof body?.requestId === "string" ? body.requestId.match(ANTIGRAVITY_CONV_RE) : null;
    return m ? normalizeSessionId(m[1]) : null;
}
function extractClientSessionId(headers: ProtocolValue, body: ProtocolRecord, scope = "") {
    ...
    if (requestId)
        return requestId;
    const fromBody = normalizeSessionId(body?.prompt_cache_key) ||
...
function requestMessages(body: ProtocolRecord) {
...
function accumulateAssistantText(body: ProtocolRecord) {
    ...
    for (const item of items) {
        ...
        if (typeof item.content === "string")
            ...
            for (const c of item.content)
                text += c?.text || c?.output || "";
        }
        if (text.length >= ASSISTANT_CAP_LEN)
            break;
    }
    return text;
}
// Stable session id keyed on accumulated assistant text (avoids collision on identical first user prompt)
function assistantTextSessionId(scope: ProtocolValue, body: ProtocolRecord) {
    const text = accumulateAssistantText(body);
    if (text.length < ASSISTANT_MIN_LEN)
        return null;
 ...
 */
export function resolveSessionIdentity({ headers, body, connectionId, workspaceId, scope = "" }: ProtocolValue = {}) {
    const client = extractClientSessionId(headers, body, scope);
    if (client)
        return { sessionId: client, ephemeral: false };
    const fromAssistant = scope === "kiro" ? null : assistantTextSessionId(`${scope}:${connectionId || ""}`, body);
    if (fromAssistant)
        return { sessionId: fromAssistant, ephemeral: false };
    const ws = normalizeSessionId(workspaceId);
    if (ws)
        return { sessionId: ws, ephemeral: false };
    if (scope === "kiro")
        return { sessionId: generateBinaryStyleId(), ephemeral: true };
    return { sessionId: deriveSessionId(connectionId), ephemeral: false };
}
export function resolveSessionId(opts: ProtocolValue = {}) {
    return resolveSessionIdentity(opts).sessionId;
}
export function resolveContinuationId({ sessionId, connectionId, scope = "", ephemeral = false }: ProtocolValue = {}) {
    if (ephemeral)
...
export function captureSessionId(body: ProtocolRecord, credentials: ProtocolCredentials, connectionId: string, scope = "") {
...
export function toNumericSessionId(sessionId: string) {
...
if (assistantCleanup.unref)
    assistantCleanup.unref();
