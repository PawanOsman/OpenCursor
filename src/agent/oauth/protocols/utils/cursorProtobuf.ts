...
const log = (tag: ProtocolValue, ...args: ProtocolValue[]) => DEBUG && (void 0);
const textDecoder = new TextDecoder();
const PROTOBUF_SCHEMA_VERSION = "1.1.3";
// ==================== SCHEMAS ====================
const WIRE_TYPE: Record<string, number> = { VARINT: 0, FIXED64: 1, LEN: 2, FIXED32: 5 };
const ROLE: Record<string, number> = { USER: 1, ASSISTANT: 2 };
const UNIFIED_MODE: Record<string, number> = { CHAT: 1, AGENT: 2 };
const THINKING_LEVEL: Record<string, number> = { UNSPECIFIED: 0, MEDIUM: 1, HIGH: 2 };
const CLIENT_SIDE_TOOL_V2: Record<string, number> = { MCP: 19 };
...
// ==================== PRIMITIVE ENCODING ====================
export function encodeVarint(value: ProtocolValue) {
    const bytes: ProtocolValue[] = [];
    while (value >= 0x80) {
...
export function encodeField(fieldNum: ProtocolValue, wireType: number, value: ProtocolValue) {
...
function concatArrays(...arrays: ProtocolValue[]) {
    const totalLength = arrays.reduce((sum: ProtocolValue, arr: ProtocolValue) => sum + arr.length, 0);
 ...
 */
function formatToolName(name: ProtocolValue) {
 ...
 */
function parseToolName(formattedName: ProtocolValue) {
 ...
 */
function parseToolId(id: ProtocolValue) {
    const delimiter = "\nmc_";
 ...
 */
function encodeMcpResult(selectedTool: ProtocolValue, resultContent: ProtocolValue) {
    return concatArrays(encodeField(FIELD.MCPR_SELECTED_TOOL, WIRE_TYPE.LEN, selectedTool), encodeField(FIELD.MCPR_RESULT, WIRE_TYPE.LEN, resultContent));
}
/**
 * Encode ClientSideToolV2Result proto: { tool, mcp_result, call_id, model_call_id, tool_index }
 * Represents the result of executing a tool
 */
function encodeClientSideToolV2Result(toolCallId: ProtocolValue, modelCallId: ProtocolValue, selectedTool: ProtocolValue, resultContent: ProtocolValue, toolIndex = 1) {
    return concatArrays(encodeField(FIELD.CV2R_TOOL, WIRE_TYPE.VARINT, CLIENT_SIDE_TOOL_V2_MCP), encodeField(FIELD.CV2R_MCP_RESULT, WIRE_TYPE.LEN, encodeMcpResult(selectedTool, resultContent)), encodeField(FIELD.CV2R_CALL_ID, WIRE_TYPE.LEN, toolCallId), ...(modelCallId ? [encodeField(FIELD.CV2R_MODEL_CALL_ID, WIRE_TYPE.LEN, modelCallId)] : []), encodeField(FIELD.CV2R_TOOL_INDEX, WIRE_TYPE.VARINT, toolIndex > 0 ? toolIndex : 1));
}
/**
 * Encode MCPParams.Tool nested inside ClientSideToolV2Call
 */
function encodeMcpParamsForCall(toolName: string, rawArgs: ProtocolValue, serverName: ProtocolValue) {
    const tool = concatArrays(encodeField(FIELD.MCP_TOOL_NAME, WIRE_TYPE.LEN, toolName), encodeField(FIELD.MCP_TOOL_PARAMS, WIRE_TYPE.LEN, rawArgs), encodeField(FIELD.MCP_TOOL_SERVER, WIRE_TYPE.LEN, serverName));
 ...
 */
function encodeClientSideToolV2Call(toolCallId: ProtocolValue, toolName: string, selectedTool: ProtocolValue, serverName: ProtocolValue, rawArgs: ProtocolValue, modelCallId: ProtocolValue, toolIndex = 1) {
    return concatArrays(encodeField(FIELD.CV2C_TOOL, WIRE_TYPE.VARINT, CLIENT_SIDE_TOOL_V2_MCP), encodeField(FIELD.CV2C_MCP_PARAMS, WIRE_TYPE.LEN, encodeMcpParamsForCall(selectedTool, rawArgs, serverName)), encodeField(FIELD.CV2C_CALL_ID, WIRE_TYPE.LEN, toolCallId), encodeField(FIELD.CV2C_NAME, WIRE_TYPE.LEN, toolName), encodeField(FIELD.CV2C_RAW_ARGS, WIRE_TYPE.LEN, rawArgs), encodeField(FIELD.CV2C_TOOL_INDEX, WIRE_TYPE.VARINT, toolIndex > 0 ? toolIndex : 1), ...(modelCallId ? [encodeField(FIELD.CV2C_MODEL_CALL_ID, WIRE_TYPE.LEN, modelCallId)] : []));
}
/**
 ...
 */
export function encodeToolResult(toolResult: ProtocolValue) {
    ...
    return concatArrays(encodeField(FIELD.TOOL_RESULT_CALL_ID, WIRE_TYPE.LEN, toolCallId), encodeField(FIELD.TOOL_RESULT_NAME, WIRE_TYPE.LEN, toolName), encodeField(FIELD.TOOL_RESULT_INDEX, WIRE_TYPE.VARINT, toolIndex > 0 ? toolIndex : 1), ...(modelCallId ? [encodeField(FIELD.TOOL_RESULT_MODEL_CALL_ID, WIRE_TYPE.LEN, modelCallId)] : []), encodeField(FIELD.TOOL_RESULT_RAW_ARGS, WIRE_TYPE.LEN, rawArgs), encodeField(FIELD.TOOL_RESULT_RESULT, WIRE_TYPE.LEN, encodeClientSideToolV2Result(toolCallId, modelCallId, selectedTool, resultContent, toolIndex)), encodeField(FIELD.TOOL_RESULT_TOOL_CALL, WIRE_TYPE.LEN, encodeClientSideToolV2Call(toolCallId, toolName, selectedTool, serverName, rawArgs, modelCallId, toolIndex)));
}
export function encodeMessage(content: ProtocolValue, role: ProtocolValue, messageId: ProtocolValue, chatModeEnum: ProtocolValue = null, isLast = false, hasTools = false, toolResults: ProtocolValue = [], serverBubbleId: ProtocolValue = null) {
    const hasToolResults = toolResults.length > 0;
    return concatArrays(encodeField(FIELD.MSG_CONTENT, WIRE_TYPE.LEN, content), encodeField(FIELD.MSG_ROLE, WIRE_TYPE.VARINT, role), encodeField(FIELD.MSG_ID, WIRE_TYPE.LEN, messageId), 
    // Only include server_bubble_id if explicitly provided (last assistant message only)
    ...(serverBubbleId ? [encodeField(FIELD.MSG_SERVER_BUBBLE_ID, WIRE_TYPE.LEN, serverBubbleId)] : []), ...(hasToolResults ? toolResults.map((tr: ProtocolValue) => encodeField(FIELD.MSG_TOOL_RESULTS, WIRE_TYPE.LEN, encodeToolResult(tr))) : []), encodeField(FIELD.MSG_IS_AGENTIC, WIRE_TYPE.VARINT, hasTools ? 1 : 0), encodeField(FIELD.MSG_UNIFIED_MODE, WIRE_TYPE.VARINT, hasTools ? UNIFIED_MODE.AGENT : UNIFIED_MODE.CHAT), ...(isLast && hasTools ? [encodeField(FIELD.MSG_SUPPORTED_TOOLS, WIRE_TYPE.LEN, encodeVarint(1))] : []));
}
export function encodeInstruction(text: ProtocolValue) {
    return text ? encodeField(FIELD.INSTRUCTION_TEXT, WIRE_TYPE.LEN, text) : new Uint8Array(0);
}
export function encodeModel(modelName: ProtocolValue) {
    return concatArrays(encodeField(FIELD.MODEL_NAME, WIRE_TYPE.LEN, modelName), encodeField(FIELD.MODEL_EMPTY, WIRE_TYPE.LEN, new Uint8Array(0)));
}
export function encodeCursorSetting() {
    const unknown6 = concatArrays(encodeField(FIELD.SETTING6_FIELD_1, WIRE_TYPE.LEN, new Uint8Array(0)), encodeField(FIELD.SETTING6_FIELD_2, WIRE_TYPE.LEN, new Uint8Array(0)));
    return concatArrays(encodeField(FIELD.SETTING_PATH, WIRE_TYPE.LEN, "cursor\\aisettings"), encodeField(FIELD.SETTING_UNKNOWN_3, WIRE_TYPE.LEN, new Uint8Array(0)), encodeField(FIELD.SETTING_UNKNOWN_6, WIRE_TYPE.LEN, unknown6), encodeField(FIELD.SETTING_UNKNOWN_8, WIRE_TYPE.VARINT, 1), encodeField(FIELD.SETTING_UNKNOWN_9, WIRE_TYPE.VARINT, 1));
}
export function encodeMetadata() {
    return concatArrays(encodeField(FIELD.META_PLATFORM, WIRE_TYPE.LEN, process.platform || "linux"), encodeField(FIELD.META_ARCH, WIRE_TYPE.LEN, process.arch || "x64"), encodeField(FIELD.META_VERSION, WIRE_TYPE.LEN, process.version || "v20.0.0"), encodeField(FIELD.META_CWD, WIRE_TYPE.LEN, process.cwd?.() || "/"), encodeField(FIELD.META_TIMESTAMP, WIRE_TYPE.LEN, new Date().toISOString()));
}
export function encodeMessageId(messageId: ProtocolValue, role: ProtocolValue, summaryId: ProtocolValue = null) {
    return concatArrays(encodeField(FIELD.MSGID_ID, WIRE_TYPE.LEN, messageId), ...(summaryId ? [encodeField(FIELD.MSGID_SUMMARY, WIRE_TYPE.LEN, summaryId)] : []), encodeField(FIELD.MSGID_ROLE, WIRE_TYPE.VARINT, role));
}
export function encodeMcpTool(tool: ProtocolValue) {
    ...
    return concatArrays(...(toolName ? [encodeField(FIELD.MCP_TOOL_NAME, WIRE_TYPE.LEN, toolName)] : []), ...(toolDesc ? [encodeField(FIELD.MCP_TOOL_DESC, WIRE_TYPE.LEN, toolDesc)] : []), ...(Object.keys(inputSchema).length > 0 ? [encodeField(FIELD.MCP_TOOL_PARAMS, WIRE_TYPE.LEN, JSON.stringify(inputSchema))] : []), encodeField(FIELD.MCP_TOOL_SERVER, WIRE_TYPE.LEN, "custom"));
}
// ==================== REQUEST BUILDING ====================
export function encodeRequest(messages: ProtocolValue, modelName: ProtocolValue, tools: ProtocolValue = [], reasoningEffort: ProtocolValue = null, forceAgentMode = false) {
    const hasTools = tools?.length > 0;
    const isAgentic = hasTools || forceAgentMode;
    const formattedMessages: ProtocolValue[] = [];
    const messageIds: ProtocolValue[] = [];
    const normalizedMessages: ProtocolValue[] = [];
    ...
    for (let i = 0; i < messages.length; i++) {
        ...
        if (msg?.role === "assistant" && hasToolCalls && hasToolResults) {
            log("ENCODE", `normalizing mixed assistant tool payload at msg[${i}] (calls=${msg.tool_calls.length}, results=${msg.tool_results.length})`);
            ...
            const nextHasToolResults = nextMsg?.role === "assistant" &&
                Array.isArray(nextMsg?.tool_results) &&
                nextMsg.tool_results.length > 0;
            const currentIds = new Set(msg.tool_results.map((tr: ProtocolValue) => tr?.tool_call_id).filter((id: ProtocolValue) => typeof id === "string"));
            const nextIds = new Set((nextMsg?.tool_results || [])
                .map((tr: ProtocolValue) => tr?.tool_call_id)
                .filter((id: ProtocolValue) => typeof id === "string"));
 ...
 */
export function buildToolResultRequest(toolResult: ProtocolValue) {
    const { toolCallId, modelCallId } = parseToolId(toolResult.tool_call_id || "");
    const rawName = toolResult.tool_name || "";
    const resultContent = toolResult.result_content || "";
    // selected_tool contains the tool name without the server prefix.
    ...
    const cv2Result = concatArrays(encodeField(FIELD.CV2R_TOOL, WIRE_TYPE.VARINT, CLIENT_SIDE_TOOL_V2_MCP), encodeField(FIELD.CV2R_MCP_RESULT, WIRE_TYPE.LEN, encodeMcpResult(selectedTool, resultContent)), encodeField(FIELD.CV2R_CALL_ID, WIRE_TYPE.LEN, toolCallId), ...(modelCallId ? [encodeField(FIELD.CV2R_MODEL_CALL_ID, WIRE_TYPE.LEN, modelCallId)] : [])
...
export function generateCursorBody(messages: ProtocolValue, modelName: ProtocolValue, tools: ProtocolValue = [], reasoningEffort: ProtocolValue = null, forceAgentMode = false) {
 ...
 */
export function generateToolResultBody(toolResult: ProtocolValue) {
    const protobuf = buildToolResultRequest(toolResult);
    return wrapConnectRPCFrame(protobuf, false);
}
// ==================== PRIMITIVE DECODING ====================
export function decodeVarint(buffer: ProtocolValue, offset: number) {
    let result = 0;
    let shift = 0;
    let pos = offset;
...
export function decodeField(buffer: ProtocolValue, offset: number) {
    if (offset >= buffer.length)
        return [null, null, null, offset];
    const [tag, pos1] = decodeVarint(buffer, offset);
    const fieldNum = tag >> 3;
    const wireType = tag & 0x07;
    let value: ProtocolValue;
    let pos = pos1;
    if (wireType === WIRE_TYPE.VARINT) {
...
export function decodeMessage(data: ProtocolValue) {
...
// ==================== RESPONSE PARSING ====================
export function parseConnectRPCFrame(buffer: ProtocolValue) {
    if (buffer.length < 5)
        return null;
    ...
    if (flags === 0x01) {
        try {
            ...
            const err = caughtError as ProtocolError;
            log("PARSE", `Decompression failed: ${err.message}`);
        }
    }
    return { flags, length, payload, consumed: 5 + length };
}
function extractToolCall(toolCallData: ProtocolValue) {
...
function extractTextAndThinking(responseData: ProtocolValue) {
    const nested = decodeMessage(responseData);
    let text: ProtocolValue = null;
    let thinking: ProtocolValue = null;
    // Extract text
...
const PB_VALUE: Record<string, number> = { NULL: 1, NUMBER: 2, STRING: 3, BOOL: 4, STRUCT: 5, LIST: 6 };
...
const MER_MESSAGE = 1;
const TNF_NAME = 1;
function asBytes(value: ProtocolValue) {
    if (!value)
 ...
 */
export function encodeAgentValue(value: ProtocolValue): Uint8Array {
    ...
    if (Array.isArray(value)) {
        const items = value.map((item: ProtocolValue) => encodeField(PB_LIST_VALUES, WIRE_TYPE.LEN, encodeAgentValue(item)));
        return encodeField(PB_VALUE.LIST, WIRE_TYPE.LEN, concatArrays(...items));
    }
    if (typeof value === "object") {
        const entries = Object.entries(value).map(([key, val]: ProtocolValue) => encodeField(PB_STRUCT_FIELDS, WIRE_TYPE.LEN, concatArrays(encodeField(PB_MAP_KEY, WIRE_TYPE.LEN, key), encodeField(PB_MAP_VALUE, WIRE_TYPE.LEN, encodeAgentValue(val)))));
 ...
 */
export function decodeAgentValue(bytes: ProtocolValue) {
    const fields = decodeMessage(asBytes(bytes));
    if (fields.has(PB_VALUE.NULL))
        return null;
    if (fields.has(PB_VALUE.BOOL))
        return fields.get(PB_VALUE.BOOL)[0].value !== 0;
    ...
    if (fields.has(PB_VALUE.STRUCT)) {
        ...
        for (const entry of decodeMessage(asBytes(fields.get(PB_VALUE.STRUCT)[0].value)).get(PB_STRUCT_FIELDS) || []) {
            ...
            if (key)
                result[key] = decodeAgentValue(pair.get(PB_MAP_VALUE)?.[0]?.value);
        }
        return result;
    }
    if (fields.has(PB_VALUE.LIST)) {
        return (decodeMessage(asBytes(fields.get(PB_VALUE.LIST)[0].value)).get(PB_LIST_VALUES) || [])
            .map((item: ProtocolValue) => decodeAgentValue(item.value));
    }
    return null;
}
function toolNameAndSchema(tool: ProtocolValue) {
 ...
 */
export function encodeMcpToolDefinition(tool: ProtocolValue) {
    const { name, description, schema } = toolNameAndSchema(tool);
    return concatArrays(encodeField(MTD_NAME, WIRE_TYPE.LEN, name), encodeField(MTD_DESCRIPTION, WIRE_TYPE.LEN, description), encodeField(MTD_INPUT_SCHEMA, WIRE_TYPE.LEN, encodeAgentValue(schema)), encodeField(MTD_PROVIDER, WIRE_TYPE.LEN, "opencursor"), encodeField(MTD_TOOL_NAME, WIRE_TYPE.LEN, name));
}
/**
 * Encode AgentRunRequest.mcp_tools: repeated McpToolDefinition under field 1.
 */
export function encodeMcpTools(tools: ProtocolValue = []) {
    if (!tools?.length)
        return new Uint8Array();
    return concatArrays(...tools.map((tool: ProtocolValue) => encodeField(MCP_TOOLS_TOOL, WIRE_TYPE.LEN, encodeMcpToolDefinition(tool))));
}
/**
 * Decode agent.v1.McpArgs (name, typed args map, toolCallId, toolName).
 */
export function decodeMcpArgs(bytes: ProtocolValue) {
    const msg = decodeMessage(asBytes(bytes));
    const args: ProtocolRecord = {};
    for (const entry of msg.get(MCP_ARGS_ENTRY) || []) {
        const pair = decodeMessage(asBytes(entry.value));
        const key = asBytes(pair.get(PB_MAP_KEY)?.[0]?.value).toString("utf8");
        if (key)
            args[key] = decodeAgentValue(pair.get(PB_MAP_VALUE)?.[0]?.value);
    }
    const read = (field: ProtocolValue) => asBytes(msg.get(field)?.[0]?.value).toString("utf8");
    return {
...
function encodeMcpTextItem(text: ProtocolValue) {
    return encodeField(MCS_CONTENT, WIRE_TYPE.LEN, encodeField(MCC_TEXT, WIRE_TYPE.LEN, encodeField(MTC_TEXT, WIRE_TYPE.LEN, text)));
}
function encodeMcpImageItem(image: ProtocolValue) {
    const data = image?.data || image || new Uint8Array();
    const mimeType = image?.mimeType || "application/octet-stream";
    return encodeField(MCS_CONTENT, WIRE_TYPE.LEN, encodeField(MCC_IMAGE, WIRE_TYPE.LEN, concatArrays(encodeField(MIC_DATA, WIRE_TYPE.LEN, data), encodeField(MIC_MIME, WIRE_TYPE.LEN, mimeType))));
}
export function encodeMcpResultSuccess({ textItems = [], imageItems = [], isError = false }: ProtocolValue = {}) {
    const success = concatArrays(...textItems.map(encodeMcpTextItem), ...imageItems.map(encodeMcpImageItem), encodeField(MCS_IS_ERROR, WIRE_TYPE.VARINT, isError ? 1 : 0));
    return encodeField(MCR_SUCCESS, WIRE_TYPE.LEN, success);
}
export function encodeMcpResultError(message: ProtocolRecord) {
    return encodeField(MCR_ERROR, WIRE_TYPE.LEN, encodeField(MER_MESSAGE, WIRE_TYPE.LEN, String(message || "")));
}
export function encodeMcpResultToolNotFound(name: ProtocolValue) {
    return encodeField(MCR_TOOL_NOT_FOUND, WIRE_TYPE.LEN, encodeField(TNF_NAME, WIRE_TYPE.LEN, String(name || "")));
}
// ==================== EXPORTS ====================
export default {
    encodeVarint,
