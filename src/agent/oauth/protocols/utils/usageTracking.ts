 ...
 */
export function addBufferToUsage(usage: ProtocolValue) {
    if (!usage || typeof usage !== "object")
        return usage;
    const result: ProtocolRecord = { ...usage };
    // Claude format
...
export function filterUsageForFormat(usage: ProtocolValue, targetFormat: string) {
    if (!usage || typeof usage !== "object")
        return usage;
    // Helper to pick only defined fields from usage
    const pickFields = (fields: ProtocolValue) => {
        const filtered: ProtocolRecord = {};
    ...
    const formatFields: ProtocolRecord = {
        [FORMATS.CLAUDE]: [
 ...
 */
export function normalizeUsage(usage: ProtocolValue) {
    if (!usage || typeof usage !== "object" || Array.isArray(usage))
        return null;
    const normalized: ProtocolRecord = {};
    const assignNumber = (key: ProtocolValue, value: ProtocolValue) => {
 ...
 */
export function canonicalizeUsage(usage: ProtocolValue) {
    if (!usage || typeof usage !== "object" || Array.isArray(usage))
        return null;
    const num = (v: ProtocolValue) => (Number.isFinite(Number(v)) ? Number(v) : 0);
 ...
 */
export function hasValidUsage(usage: ProtocolValue) {
    if (!usage || typeof usage !== "object")
        return false;
 ...
 */
export function extractUsage(chunk: ProtocolValue) {
...
export function mergeUsage(prev: ProtocolValue, next: ProtocolValue) {
    if (!prev)
        return next || null;
    if (!next)
        return prev;
    const merged: ProtocolRecord = { ...prev };
 ...
 */
export function estimateInputTokens(body: ProtocolRecord) {
 ...
 */
export function estimateOutputTokens(contentLength: ProtocolValue) {
 ...
 */
export function formatUsage(inputTokens: ProtocolValue, outputTokens: ProtocolValue, targetFormat: string) {
 ...
 */
export function estimateUsage(body: ProtocolRecord, contentLength: ProtocolValue, targetFormat: string = FORMATS.OPENAI) {
    return formatUsage(estimateInputTokens(body), estimateOutputTokens(contentLength), targetFormat);
}
/**
 * Log usage with cache info (green color)
 */
export function logUsage(provider: string, usage: ProtocolValue, model: ProtocolValue = null, connectionId: ProtocolValue = null, apiKey: ProtocolValue = null) {
    if (!usage || typeof usage !== "object")
        return;
    ...
    if (cacheCreation)
        msg += ` | cache_create=${cacheCreation}`;
