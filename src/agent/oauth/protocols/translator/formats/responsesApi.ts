 ...
 */
export function normalizeResponsesInput(input: ProtocolValue) {
...
export function clampResponsesCallId(id: ProtocolValue) {
    if (typeof id !== "string" || !id)
        return `call_${Date.now()}_${(responsesCallIdSeq += 1)}`;
...
export function coerceResponsesArguments(value: ProtocolValue) {
...
export function coerceResponsesOutput(value: ProtocolValue) {
    if (typeof value === "string")
        return value;
    if (value === undefined || value === null)
        return "";
    if (Array.isArray(value)) {
        return value.map((c: ProtocolValue) => {
            try {
 ...
 */
export function convertResponsesApiFormat(body: ProtocolRecord) {
    if (!body.input)
        return body;
    ...
    let pendingToolCalls: ProtocolValue[] = [];
