...
export const UNSUPPORTED_SCHEMA_CONSTRAINTS: string[] = [
...
export const DEFAULT_SAFETY_SETTINGS: ProtocolValue[] = [
...
export function convertOpenAIContentToParts(content: ProtocolValue) {
...
export function extractTextContent(content: ProtocolValue, separator = "") {
    if (typeof content === "string")
        return content;
    if (Array.isArray(content)) {
        return content.filter((c: ProtocolValue) => c.type === OPENAI_BLOCK.TEXT).map((c: ProtocolValue) => c.text).join(separator);
    }
    return "";
}
// Try parse JSON safely (null fallback on parse error; re-export keeps legacy API)
export function tryParseJSON(str: ProtocolValue) {
...
export function generateProjectId() {
    const adjectives: string[] = ["useful", "bright", "swift", "calm", "bold"];
    const nouns: string[] = ["fuze", "wave", "spark", "flow", "core"];
...
function removeUnsupportedKeywords(obj: ProtocolValue, keywords: ProtocolValue) {
...
function convertConstToEnum(obj: ProtocolValue) {
    if (!obj || typeof obj !== "object")
        return;
...
function convertEnumValuesToStrings(obj: ProtocolValue) {
    if (!obj || typeof obj !== "object")
        return;
    if (obj.enum && Array.isArray(obj.enum)) {
        obj.enum = obj.enum.map((v: ProtocolValue) => String(v));
...
// Merge allOf schemas
function mergeAllOf(obj: ProtocolValue) {
    if (!obj || typeof obj !== "object")
        return;
...
function selectBest(items: ProtocolValue) {
    let bestIdx = 0;
    let bestScore = -1;
...
// Flatten anyOf/oneOf
function flattenAnyOfOneOf(obj: ProtocolValue) {
    if (!obj || typeof obj !== "object")
        return;
    if (obj.anyOf && Array.isArray(obj.anyOf) && obj.anyOf.length > 0) {
        const nonNullSchemas = obj.anyOf.filter((s: ProtocolValue) => s && s.type !== "null");
    ...
    if (obj.oneOf && Array.isArray(obj.oneOf) && obj.oneOf.length > 0) {
        const nonNullSchemas = obj.oneOf.filter((s: ProtocolValue) => s && s.type !== "null");
        if (nonNullSchemas.length > 0) {
            const bestIdx = selectBest(nonNullSchemas);
            const selected = nonNullSchemas[bestIdx];
            delete obj.oneOf;
...
// Flatten type arrays
function flattenTypeArrays(obj: ProtocolValue) {
    if (!obj || typeof obj !== "object")
        return;
    if (obj.type && Array.isArray(obj.type)) {
        const nonNullTypes = obj.type.filter((t: ProtocolValue) => t !== "null");
...
function ensureObjectType(obj: ProtocolValue) {
    if (!obj || typeof obj !== "object")
        return;
...
function convertPrefixItems(obj: ProtocolValue) {
    if (!obj || typeof obj !== "object")
        return;
    if (Array.isArray(obj.prefixItems) && obj.prefixItems.length > 0) {
        const variants = obj.prefixItems.filter((s: ProtocolValue) => s && s.type !== "null");
...
function ensureArrayItems(obj: ProtocolValue) {
    if (!obj || typeof obj !== "object")
        return;
...
export function cleanJSONSchemaForAntigravity(schema: ProtocolValue) {
    ...
    function cleanupRequired(obj: ProtocolValue) {
        if (!obj || typeof obj !== "object")
            return;
        if (obj.required && Array.isArray(obj.required) && obj.properties) {
            const validRequired = obj.required.filter((field: ProtocolValue) => Object.prototype.hasOwnProperty.call(obj.properties, field));
    ...
    function addPlaceholders(obj: ProtocolValue) {
        if (!obj || typeof obj !== "object")
            return;
...
export function normalizeGeminiContents(contents: ProtocolValue) {
    const out: ProtocolValue[] = [];
    for (const c of contents || []) {
        if (!c?.role || !Array.isArray(c.parts))
            continue;
        const parts = c.parts.filter((p: ProtocolValue) => p && Object.keys(p).length > 0);
        if (parts.length === 0)
            continue;
        const last = out.at(-1);
        if (last?.role === c.role)
