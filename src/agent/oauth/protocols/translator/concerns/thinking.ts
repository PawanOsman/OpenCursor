...
export const EFFORT_LEVELS: string[] = ["minimal", "low", "medium", "high", "xhigh", "max"];
...
export function effortToBudget(effort: ProtocolValue) {
    if (!effort)
        return undefined;
...
export function effortToThinkingLevel(effort: ProtocolValue) {
...
export function budgetToLevel(budget: number) {
...
export function budgetToEffort(budget: number) {
    if (!budget || budget <= 0)
        return null;
    if (budget <= 2048)
        return "low";
