...
export function toOpenAIFinish(reason: ProtocolValue, format: string) {
    switch (format) {
        case "claude":
            switch (reason) {
                ...
                // OpenAI client saw finish_reason "stop" and an empty message (OpenCursor logged
...
export function fromOpenAIFinish(reason: ProtocolValue, format: string) {
    switch (format) {
        case "claude":
            switch (reason) {
