...
const STRIP_RULES: ProtocolValue[] = [
    ...
    { provider: "github", match: (m: ProtocolValue) => /claude/i.test(m) && !/claude.*(opus|sonnet).*4\.6/i.test(m), drop: ["thinking", "reasoning_effort"] },
...
function matches(rule: ProtocolValue, model: string) {
    if (!rule.match)
        return true;
    return typeof rule.match === "function" ? rule.match(model) : rule.match.test(model);
}
function clampNumber(body: ProtocolRecord, key: ProtocolValue, ceiling: ProtocolValue) {
...
export function stripUnsupportedParams(provider: string, model: string, body: ProtocolRecord) {
    ...
    for (const rule of STRIP_RULES) {
            ...
            for (const msg of body.messages) {
                if (msg && Array.isArray(msg.content)) {
                    msg.content = msg.content
                        .map((b: ProtocolValue) => (b?.type === "text" && typeof b.text === "string") ? b.text : "")
                        .join("");
                }
            }
        }
        if (rule.clampToModelMaxOutput || Number.isFinite(rule.maxOutputCap)) {
            ...
            const candidates: ProtocolValue[] = [];
