...
import { KIRO_TOOL_DESCRIPTION_MAX_LENGTH, KIRO_TOOL_ID_MAX_LENGTH, KIRO_TOOL_NAME_MAX_LENGTH, } from "../../config/kiroConstants.js";
...
export function kiroEmptyUserContent(hasToolResults: ProtocolValue) {
...
function text(value: ProtocolValue) {
...
function appendText(target: ProtocolValue, extra: ProtocolValue) {
    if (!extra)
        return;
    target.content = target.content ? `${target.content}\n\n${extra}` : extra;
}
function trimCodePoints(value: ProtocolValue, limit: ProtocolValue) {
    return [...String(value || "")].slice(0, limit).join("");
}
function uniqueName(rawName: ProtocolValue, index: number, usedNames: ProtocolValue) {
...
function cleanSchemaValue(value: ProtocolValue): ProtocolValue {
    if (Array.isArray(value))
        return value.map(cleanSchemaValue);
    if (!value || typeof value !== "object")
        return value;
    const cleaned: ProtocolRecord = {};
...
function normalizeRootSchema(schema: ProtocolValue) {
    ...
    if (Array.isArray(cleaned.required)) {
        cleaned.required = [...new Set(cleaned.required.filter((name: ProtocolValue) => typeof name === "string" && Object.hasOwn(cleaned.properties, name)))];
...
export function normalizeKiroToolSpecs(tools: ProtocolValue) {
    const specs: ProtocolValue[] = [];
...
function toolCallText(toolUse: ProtocolValue) {
    return `[Tool call: ${toolUse?.name || "unknown"}(${text(toolUse?.input || {})})]`;
}
function toolResultText(toolResult: ProtocolValue) {
    const content = Array.isArray(toolResult?.content)
        ? toolResult.content.map((part: ProtocolValue) => text(part?.text ?? part)).filter(Boolean).join("\n")
        : text(toolResult?.content);
    return `[Tool result${toolResult?.status === "error" ? " (error)" : ""}: ${content}]`;
}
function mergeUser(target: ProtocolValue, source: ProtocolValue) {
...
function mergeAssistant(target: ProtocolValue, source: ProtocolValue) {
    appendText(target, source.content);
    if (Array.isArray(source.toolUses) && source.toolUses.length > 0) {
        target.toolUses = [...(target.toolUses || []), ...source.toolUses];
    }
}
function normalizeTurns(history: ProtocolValue, currentMessage: ProtocolValue, modelId: string) {
    const rawTurns: ProtocolValue[] = [...(Array.isArray(history) ? history : [])];
    if (currentMessage)
        rawTurns.push(currentMessage);
    const turns: ProtocolValue[] = [];
...
function rawId(value: ProtocolValue) {
    return typeof value === "string" ? value : "";
}
function reserveToolId(value: ProtocolValue, turnIndex: ProtocolValue, callIndex: ProtocolValue, name: ProtocolValue, usedIds: ProtocolValue) {
    const sanitized = rawId(value).replace(/[^a-zA-Z0-9_-]/g, "");
    const generated = `call_msg${turnIndex}_tc${callIndex}_${name || "tool"}`;
    const base = trimCodePoints(TOOL_ID_PATTERN.test(sanitized) && sanitized ? sanitized : generated, KIRO_TOOL_ID_MAX_LENGTH);
    let candidate = base;
    let suffix = 2;
...
function normalizeToolInput(input: ProtocolValue) {
...
function normalizeToolResult(result: ProtocolValue) {
    const content = Array.isArray(result?.content)
        ? result.content.map((part: ProtocolValue) => ({ text: text(part?.text ?? part) }))
...
function flattenResults(userMessage: ProtocolValue, results: ProtocolValue) {
    for (const result of results)
        appendText(userMessage, toolResultText(result));
}
function cleanUserContext(userMessage: ProtocolValue) {
...
function reconcileToolPair(assistant: ProtocolValue, user: ProtocolValue, turnIndex: ProtocolValue, nameMap: ProtocolValue, specNames: ProtocolValue, usedIds: ProtocolValue, repairs: ProtocolValue) {
    ...
    const callRecords = calls.map((call: ProtocolValue, callIndex: ProtocolValue) => {
        const key = rawId(call?.toolUseId);
        const mappedName = nameMap.get(call?.name) || call?.name;
        const input = normalizeToolInput(call?.input);
        const record: ProtocolRecord = { call, callIndex, key, mappedName, input, result: null };
        const queue = callQueues.get(key) || [];
        queue.push(record);
        callQueues.set(key, queue);
        return record;
    });
    const orphanResults: ProtocolValue[] = [];
    for (const result of results) {
        const queue = callQueues.get(rawId(result.toolUseId));
        const record = queue?.find((candidate: ProtocolValue) => !candidate.result);
        if (record)
            record.result = result;
        else
            orphanResults.push(result);
    }
    const keptCalls: ProtocolValue[] = [];
    const keptResults: ProtocolValue[] = [];
    for (const record of callRecords) {
        ...
        const toolUseId = reserveToolId(record.key, turnIndex, record.callIndex, record.mappedName, usedIds);
        keptCalls.push({
            toolUseId,
    ...
    if (keptResults.length > 0)
        user.userInputMessageContext.toolResults = keptResults;
    else
        delete user.userInputMessageContext.toolResults;
    cleanUserContext(user);
}
/** Validate the final Kiro wire conversation without mutating it. */
export function validateKiroConversation(history: ProtocolValue, currentMessage: ProtocolValue, toolSpecs: ProtocolValue = []) {
    const errors: ProtocolValue[] = [];
    const turns = [...(history || []), currentMessage].filter(Boolean);
    const specNames = new Set(toolSpecs.map((spec: ProtocolValue) => spec?.toolSpecification?.name).filter(Boolean));
    const usedIds = new Set();
    for (let index = 0; index < turns.length; index++) {
        const expectedUser = index % 2 === 0;
        const isUser = !!turns[index]?.userInputMessage;
        if (isUser !== expectedUser)
            errors.push(`role:${index}`);
        if (!isUser) {
            ...
            const callIds = calls.map((call: ProtocolValue) => call.toolUseId);
            const resultIds = results.map((result: ProtocolValue) => result.toolUseId);
            if (calls.length !== results.length || callIds.some((id: ProtocolValue) => !resultIds.includes(id))) {
                errors.push(`pair:${index}`);
            }
            for (const call of calls) {
                if (!call.toolUseId || usedIds.has(call.toolUseId))
                    errors.push(`id:${index}`);
                usedIds.add(call.toolUseId);
                if (!specNames.has(call.name))
                    errors.push(`spec:${index}`);
            }
        }
        else if (index === 0) {
            const results = turns[index].userInputMessage?.userInputMessageContext?.toolResults;
            if (results?.length)
                errors.push("orphan:0");
        }
    }
...
function flattenAllStructuredTools(turns: ProtocolValue, repairs: ProtocolValue) {
    for (const turn of turns) {
 ...
 */
export function canonicalizeKiroConversation({ history, currentMessage, modelId, toolSpecs = [], nameMap = new Map(), }: ProtocolValue = {}) {
    const turns = normalizeTurns(history, currentMessage, modelId);
    const repairs: Record<string, number> = { missingResults: 0, orphanResults: 0, invalidToolUses: 0 };
    const specNames = new Set(toolSpecs.map((spec: ProtocolValue) => spec?.toolSpecification?.name).filter(Boolean));
    const usedIds = new Set();
