...
export function parseSuffix(model: string) {
    if (typeof model !== "string")
    ...
    if (raw === "auto")
        return { cleanModel, override: { mode: "auto" } };
    if (raw === "ultra")
...
export function extractThinking(body: ProtocolRecord) {
...
function resolveFormat(targetFormat: string, model: string, provider: string) {
...
function toBudget(cfg: ProtocolValue, range: ProtocolValue) {
    let budget: ProtocolValue;
    if (cfg.mode === "budget")
        budget = cfg.budget;
    ...
    if (!Number.isFinite(budget))
        return undefined;
    if (range) {
        if (range.min != null && budget < range.min)
            budget = range.min;
        if (range.max != null && budget > range.max)
            budget = range.max;
    }
    return budget;
}
// Convert unified config to a discrete level string.
function toLevel(cfg: ProtocolValue) {
    if (cfg.mode === "level")
        return cfg.level;
    if (cfg.mode === "budget")
        return budgetToLevel(cfg.budget) || "medium";
    if (cfg.mode === "auto")
        return "auto";
    return null;
}
function normalizeOpenAILevel(level: ProtocolValue, supportedLevels: ProtocolValue) {
    if (level !== "max" && level !== "ultra")
        return level;
    if (supportedLevels?.includes(level))
        return level;
    if (level === "ultra" && supportedLevels?.includes("max"))
        return "max";
    return "xhigh";
}
function toGeminiThinkingLevel(cfg: ProtocolValue) {
    const raw = cfg.mode === "auto" ? "high" : (toLevel(cfg) || "high");
    return effortToThinkingLevel(raw);
}
function toKimiReasoningEffort(cfg: ProtocolValue) {
...
function geminiBudgetOutputFloor(budget: number) {
    if (budget === -1)
        return 32768;
    if (!Number.isFinite(budget))
        return 32768;
...
function geminiLevelOutputFloor(level: ProtocolValue) {
...
function getGeminiGenerationConfig(body: ProtocolRecord) {
...
function setGeminiThinking(body: ProtocolRecord, tc: ProtocolValue) {
    const gc = getGeminiGenerationConfig(body);
    gc.thinkingConfig = tc;
}
function ensureGeminiOutputFloor(body: ProtocolRecord, floor: ProtocolValue, caps: ProtocolValue) {
...
function stripAll(body: ProtocolRecord) {
...
function applyFormat(fmt: ProtocolValue, body: ProtocolRecord, cfg: ProtocolValue, caps: ProtocolValue, supportedLevels: ProtocolValue, display: ProtocolValue) {
    ...
    switch (fmt) {
        ...
        case "step": {
            ...
            const level = toLevel(eff);
            if (level)
                body.reasoning_effort = level === "xhigh" || level === "max" ? "high" : level;
            break;
        }
        case "tokenrouter": {
...
export function applyThinking(targetFormat: string, model: string, body: ProtocolRecord, provider: ProtocolValue = null, intent: ProtocolValue = undefined) {
    if (!body || typeof body !== "object")
        return body;
