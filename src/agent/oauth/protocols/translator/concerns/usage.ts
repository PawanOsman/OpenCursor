...
export function buildUsage({ promptTokens, completionTokens, totalTokens, cachedTokens = 0, cacheCreationTokens = 0, reasoningTokens = 0 }: ProtocolValue) {
    const usage: ProtocolRecord = { prompt_tokens: promptTokens, completion_tokens: completionTokens, total_tokens: totalTokens };
...
const n = (v: ProtocolValue) => (typeof v === "number" ? v : 0);
...
const USAGE_EXTRACTORS: ProtocolRecord = {
    claude(raw: ProtocolValue) {
    ...
    kiro(raw: ProtocolValue) {
        ...
        const out: ProtocolRecord = { promptTokens: input, completionTokens: output, totalTokens: input + output };
        if (cached > 0)
...
export function toOpenAIUsage(raw: ProtocolValue, kind: ProtocolValue) {
    const extract = USAGE_EXTRACTORS[kind];
    if (!extract || !raw || typeof raw !== "object")
        return null;
