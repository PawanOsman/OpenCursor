...
function sanitizeToolId(id: ProtocolValue) {
...
export function ensureToolCallIds(body: ProtocolRecord) {
...
export function getToolCallIds(msg: ProtocolValue) {
    if (msg.role !== "assistant")
        return [];
    const ids: ProtocolValue[] = [];
...
export function hasToolResults(msg: ProtocolValue, toolCallIds: ProtocolValue) {
    if (!msg || !toolCallIds.length)
        return false;
...
export function fixMissingToolResponses(body: ProtocolRecord) {
    if (!body.messages || !Array.isArray(body.messages))
        return body;
    const newMessages: ProtocolValue[] = [];
    for (let i = 0; i < body.messages.length; i++) {
        const msg = body.messages[i];
...
export function defaultClaudeToolType(tools: ProtocolValue) {
    if (!Array.isArray(tools))
        return tools;
    return tools.map((tool: ProtocolValue) => tool?.type ? tool : { ...tool, type: "custom" });
}
...
export function shouldDefaultClaudeToolType(provider: string, finalFormat: ProtocolValue, tools: ProtocolValue, PROVIDERS: ProtocolValue) {
