 ...
 */
export function ollamaToOpenAIResponse(chunk: ProtocolValue, state: TranslatorState) {
 ...
 */
function extractUsage(ollamaChunk: ProtocolValue) {
    return toOpenAIUsage(ollamaChunk, "ollama");
}
/**
 * Convert tool_calls from Ollama format to OpenAI format
 */
function convertToolCalls(toolCalls: ProtocolValue) {
    return toolCalls.map((tc: ProtocolValue, i: ProtocolValue) => ({
 ...
 */
export function ollamaBodyToOpenAI(body: ProtocolRecord) {
    const msg = body.message || {};
    const content = msg.content || "";
    const thinking = msg.thinking || "";
    const toolCalls = Array.isArray(msg.tool_calls) ? msg.tool_calls : [];
    const message: ProtocolRecord = { role: ROLE.ASSISTANT };
    if (content)
        message.content = content;
    if (thinking)
        message.reasoning_content = thinking;
    if (toolCalls.length > 0)
        message.tool_calls = convertToolCalls(toolCalls);
