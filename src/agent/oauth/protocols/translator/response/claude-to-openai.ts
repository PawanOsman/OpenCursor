...
function createChunk(state: TranslatorState, delta: ProtocolRecord, finishReason: ProtocolValue = null) {
    return buildChunk({ id: `chatcmpl-${state.messageId}`, created: Math.floor(Date.now() / 1000), model: state.model }, delta, finishReason);
}
// Convert Claude stream chunk to OpenAI format
export function claudeToOpenAIResponse(chunk: ProtocolValue, state: TranslatorState) {
    if (!chunk)
        return null;
    const results: ProtocolValue[] = [];
...
const convertStopReason = (reason: ProtocolValue) => toOpenAIFinish(reason, "claude");
// Register
