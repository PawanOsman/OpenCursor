 ...
 * already split per line by the upstream SSE/JSON-line reader in OpenCursor.
 */
...
function ensureState(state: TranslatorState, model: string) {
    if (!state.responseId) {
...
function makeChunk(state: TranslatorState, delta: ProtocolRecord, finishReason: ProtocolValue = null) {
    return buildChunk({ id: state.responseId, created: state.created, model: state.model }, delta, finishReason);
}
const mapFinishReason = (reason: ProtocolValue) => toOpenAIFinish(reason, "commandcode");
export function commandCodeToOpenAIResponse(chunk: ProtocolValue, state: TranslatorState) {
    if (!chunk)
        return null;
