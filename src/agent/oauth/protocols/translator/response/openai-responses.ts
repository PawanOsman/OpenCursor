...
import type { ProtocolValue, ProtocolRecord, TranslatorState } from "../../wireTypes.js";
/**
...
// session grows until the upstream context limit rejects it (OpenCursor issue #3432).
//
...
function toResponsesUsage(usage: ProtocolValue) {
    ...
    const responseUsage: ProtocolRecord = {
...
export function openaiToOpenAIResponsesResponse(chunk: ProtocolValue, state: TranslatorState) {
    if (!chunk) {
    ...
    if (!chunk.choices?.length)
        return [];
    const events: ProtocolValue[] = [];
    const nextSeq = () => ++state.seq;
    const emit = (eventType: ProtocolValue, data: ProtocolValue) => {
    ...
    if (delta.content) {
        ...
        if (content.includes("</think>")) {
            ...
            if (thinkPart)
                emitReasoningDelta(state, emit, thinkPart);
    ...
    if (choice.finish_reason) {
        for (const i in state.msgItemAdded)
            closeMessage(state, emit, i);
        closeReasoning(state, emit);
        for (const i in state.funcCallIds)
            closeToolCall(state, emit, i);
...
// Helper functions
function startReasoning(state: TranslatorState, emit: ProtocolValue, idx: ProtocolValue) {
...
function emitReasoningDelta(state: TranslatorState, emit: ProtocolValue, text: ProtocolValue) {
    if (!text)
        return;
...
function closeReasoning(state: TranslatorState, emit: ProtocolValue) {
...
function emitTextContent(state: TranslatorState, emit: ProtocolValue, idx: ProtocolValue, content: ProtocolValue) {
...
function closeMessage(state: TranslatorState, emit: ProtocolValue, idx: ProtocolValue) {
...
function isCustomTool(state: TranslatorState, name: ProtocolValue) {
    return !!name && state.customToolNames?.has(name);
}
function extractCustomToolInput(argumentsText: ProtocolValue) {
...
function emitToolCall(state: TranslatorState, emit: ProtocolValue, tc: ProtocolValue) {
...
function closeToolCall(state: TranslatorState, emit: ProtocolValue, idx: ProtocolValue) {
...
function sendCompleted(state: TranslatorState, emit: ProtocolValue) {
...
function flushEvents(state: TranslatorState) {
    if (state.completedSent)
        return [];
    const events: ProtocolValue[] = [];
    const nextSeq = () => ++state.seq;
...
function computeFinishReason(state: TranslatorState) {
 ...
 */
export function openaiResponsesToOpenAIResponse(chunk: ProtocolValue, state: TranslatorState) {
    if (!chunk) {
        ...
        const finalChunk = buildChunk({ id: state.chatId || `chatcmpl-${Date.now()}`, created: state.created || Math.floor(Date.now() / 1000), model: state.model || MODEL_FALLBACK }, {}, finishReason);
    ...
    if (eventType === "response.output_text.delta") {
        ...
        if (!delta)
            return null;
        return buildChunk({ id: state.chatId, created: state.created, model: state.model || MODEL_FALLBACK }, { content: delta });
    }
    ...
    if (eventType === "response.output_item.added" && (data.item?.type === RESPONSES_ITEM.FUNCTION_CALL || data.item?.type === "custom_tool_call")) {
        ...
        if (key && state.respToolChatIndex.has(key)) {
            ...
            if (key)
                state.respToolChatIndex.set(key, idx);
        }
        return buildChunk({ id: state.chatId, created: state.created, model: state.model || MODEL_FALLBACK }, {
            tool_calls: [{
                    index: idx,
    ...
    if (eventType === "response.function_call_arguments.delta" || eventType === "response.custom_tool_call_input.delta") {
        ...
        return buildChunk({ id: state.chatId, created: state.created, model: state.model || MODEL_FALLBACK }, { tool_calls: [{ index: idx, function: { arguments: argsDelta } }] });
    }
    ...
    if (eventType === "response.output_item.done" && (data.item?.type === RESPONSES_ITEM.FUNCTION_CALL || data.item?.type === "custom_tool_call")) {
        ...
        if (typeof fullArgs === "string" && fullArgs) {
            ...
            if (!state.respToolArgsEmitted.has(idx)) {
                state.respToolArgsEmitted.add(idx);
                return buildChunk({ id: state.chatId, created: state.created, model: state.model || MODEL_FALLBACK }, { tool_calls: [{ index: idx, function: { arguments: fullArgs } }] });
            }
        }
        return null;
    }
    ...
    if (eventType === "response.completed" || eventType === "response.done") {
        ...
        if (!state.finishReasonSent) {
            ...
            const finalChunk = buildChunk({ id: state.chatId, created: state.created, model: state.model || MODEL_FALLBACK }, {}, finishReason);
    ...
    if (eventType === "error" || eventType === "response.failed") {
        ...
        if (error) {
            ...
            return buildChunk({ id: state.chatId || `chatcmpl-${Date.now()}`, created: state.created || Math.floor(Date.now() / 1000), model: state.model || MODEL_FALLBACK }, { content: `[Error] ${error.message || JSON.stringify(error)}` }, OPENAI_FINISH.STOP);
        }
        return null;
    }
    ...
    if (eventType === "response.reasoning_summary_text.delta") {
        ...
        if (!delta)
            return null;
        return buildChunk({ id: state.chatId, created: state.created, model: state.model || MODEL_FALLBACK }, reasoningDelta(delta));
    }
    // Ignore other events
    return null;
}
