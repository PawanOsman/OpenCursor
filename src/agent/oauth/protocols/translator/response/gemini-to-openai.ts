...
function chunkMeta(state: TranslatorState) {
...
function emitFunctionCall(functionCall: ProtocolValue, state: TranslatorState, signature: ProtocolValue = null) {
...
export function geminiToOpenAIResponse(chunk: ProtocolValue, state: TranslatorState) {
    if (!chunk)
        return null;
    ...
    if (content?.parts) {
        for (const part of content.parts) {
            ...
            if (hasThoughtSig) {
                ...
                if (hasTextContent) {
                    results.push(buildChunk(chunkMeta(state), isThought ? reasoningDelta(part.text) : { content: part.text }, null));
                }
                if (hasFunctionCall) {
            ...
            if (inlineData?.data) {
                ...
                results.push(buildChunk(chunkMeta(state), {
                    images: [{
