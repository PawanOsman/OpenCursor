...
import { DEFAULT_SAFETY_SETTINGS, convertOpenAIContentToParts, extractTextContent, tryParseJSON, generateRequestId, generateSessionId, generateProjectId, cleanJSONSchemaForAntigravity, normalizeGeminiContents } from "../formats/gemini.js";
...
function sanitizeGeminiFunctionName(name: ProtocolValue) {
    if (!name)
        return "_unknown";
...
function openaiToGeminiBase(model: string, body: ProtocolRecord, stream: boolean, signature: ProtocolValue = DEFAULT_THINKING_AG_SIGNATURE, sessionId: ProtocolValue = null) {
    ...
    const tcID2Name: ProtocolRecord = {};
    ...
    const toolResponses: ProtocolRecord = {};
    if (body.messages && Array.isArray(body.messages)) {
        for (const msg of body.messages) {
        ...
        for (let i = 0; i < body.messages.length; i++) {
            ...
            if (role === ROLE.SYSTEM && body.messages.length > 1) {
                ...
                if (msg.tool_calls && Array.isArray(msg.tool_calls)) {
                    const toolCallIds: ProtocolValue[] = [];
                    ...
                    const hasActualResponses = toolCallIds.some((fid: ProtocolValue) => toolResponses[fid] !== undefined);
                    if (hasActualResponses || isIntermediate) {
                        const toolParts: ProtocolValue[] = [];
    ...
    if (body.tools && Array.isArray(body.tools) && body.tools.length > 0) {
        const functionDeclarations: ProtocolValue[] = [];
...
export function openaiToGeminiRequest(model: string, body: ProtocolRecord, stream: boolean, credentials: ProtocolValue = null) {
    return openaiToGeminiBase(model, body, stream, DEFAULT_THINKING_AG_SIGNATURE, credentials?._clientSessionId);
}
// OpenAI -> Gemini CLI (Cloud Code Assist)
export function openaiToGeminiCLIRequest(model: string, body: ProtocolRecord, stream: boolean, credentials: ProtocolValue = null) {
...
function wrapInCloudCodeEnvelope(model: string, geminiCLI: ProtocolValue, credentials: ProtocolValue = null, isAntigravity = false) {
...
function wrapInCloudCodeEnvelopeForClaude(model: string, claudeRequest: ProtocolValue, credentials: ProtocolValue = null, signature: ProtocolValue = DEFAULT_THINKING_AG_SIGNATURE) {
    const projectId = credentials?.projectId || generateProjectId();
    const envelope: ProtocolRecord = {
        project: projectId,
        model: model,
    ...
    const toolUseIdToName: ProtocolRecord = {};
                ...
                for (const block of msg.content) {
                    if (block.type === CLAUDE_BLOCK.TEXT) {
                        ...
                        if (Array.isArray(content)) {
                            content = content.map((c: ProtocolValue) => c.type === CLAUDE_BLOCK.TEXT ? c.text : JSON.stringify(c)).join("\n");
                        }
    ...
    const systemParts: ProtocolValue[] = [];
...
export function openaiToAntigravityRequest(model: string, body: ProtocolRecord, stream: boolean, credentials: ProtocolValue = null) {
...
register(FORMATS.OPENAI, FORMATS.GEMINI_CLI, (model: ProtocolValue, body: ProtocolValue, stream: ProtocolValue, credentials: ProtocolValue) => wrapInCloudCodeEnvelope(model, openaiToGeminiCLIRequest(model, body, stream), credentials), null);
