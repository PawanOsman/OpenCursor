...
function flattenText(content: ProtocolValue) {
    if (content == null)
        return "";
...
function toNativeImageBlock(part: ProtocolValue) {
...
function toContentBlocks(content: ProtocolValue) {
    if (content == null)
        return [{ type: OPENAI_BLOCK.TEXT, text: "" }];
    if (typeof content === "string")
...
function safeParseJson(s: ProtocolValue) {
    if (s == null)
        return {};
    if (typeof s !== "string")
        return s;
...
function convertMessages(messages: ProtocolValue = []) {
    ...
    const systemTexts: ProtocolValue[] = [];
    for (const m of messages) {
        ...
        if (role === ROLE.ASSISTANT) {
            ...
            if (text)
                blocks.push({ type: OPENAI_BLOCK.TEXT, text });
...
function convertTools(tools: ProtocolValue) {
...
export function openaiToCommandCodeRequest(model: string, body: ProtocolRecord, stream: boolean /* , credentials */) {
