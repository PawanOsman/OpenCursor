...
import { normalizeResponsesInput, clampResponsesCallId, coerceResponsesArguments, coerceResponsesOutput, } from "../formats/responsesApi.js";
import { ROLE, OPENAI_BLOCK, RESPONSES_ITEM } from "../schema/index.js";
const MAX_TOOL_NAME_LEN = 128;
/**
 * Convert OpenAI Responses API request to OpenAI Chat Completions format
 */
export function openaiResponsesToOpenAIRequest(model: string, body: ProtocolRecord, stream: boolean, credentials: ProtocolCredentials) {
    if (!body.input)
        return body;
    const result: ProtocolRecord = { ...body };
    result.messages = [];
    ...
    let currentAssistantMsg: ProtocolValue = null;
    let pendingToolResults: ProtocolValue[] = [];
    let pendingReasoning = "";
    let pendingReasoningEncrypted = "";
    const additionalTools: ProtocolValue[] = [];
    const customToolNames = new Set();
    const inputItems = normalizeResponsesInput(body.input);
    if (!inputItems)
        return body;
    // Extract reasoning text from summary[].text (encrypted_content is continuity-only)
    const extractReasoningText = (item: ProtocolValue) => {
        if (Array.isArray(item.summary)) {
            const txt = item.summary.map((s: ProtocolValue) => s?.text || "").filter(Boolean).join("\n");
            if (txt)
                return txt;
        }
        if (Array.isArray(item.content)) {
            const txt = item.content.map((c: ProtocolValue) => c?.text || "").filter(Boolean).join("\n");
            if (txt)
                return txt;
        }
        return "";
    };
    const attachPendingReasoning = (msg: ProtocolValue) => {
        if (pendingReasoning)
    ...
    for (const item of inputItems) {
        ...
        if (itemType === RESPONSES_ITEM.MESSAGE) {
            ...
            const content = Array.isArray(item.content)
                ? item.content.map((c: ProtocolValue) => {
                ...
                : item.content;
            const msg: ProtocolRecord = { role: item.role, content };
    ...
    const responseTools: ProtocolValue[] = [
        ...(Array.isArray(body.tools) ? body.tools : []),
        ...additionalTools,
    ];
    if (responseTools.length > 0) {
        result.tools = responseTools
            .map((tool: ProtocolValue) => {
 ...
 */
function extractInstructionsText(content: ProtocolValue) {
    if (typeof content === "string")
        return content;
    if (Array.isArray(content)) {
        return content.map((c: ProtocolValue) => {
 ...
 */
function normalizeToolParameters(params: ProtocolValue) {
    if (!params)
 ...
 */
function buildReasoningInputItem(msg: ProtocolValue) {
    if (!msg || typeof msg !== "object")
        return null;
    const encrypted = (typeof msg.encrypted_content === "string" && msg.encrypted_content) ||
    ...
    if (typeof msg.reasoning_content === "string" && msg.reasoning_content.trim()) {
        ...
        summaryText = msg.reasoning_details
            .map((d: ProtocolValue) => (typeof d?.text === "string" ? d.text : typeof d?.content === "string" ? d.content : ""))
            .filter(Boolean)
            .join("\n");
    }
    if (!encrypted && !summaryText)
        return null;
    const item: ProtocolRecord = { type: RESPONSES_ITEM.REASONING };
    if (summaryText) {
 ...
 */
export function openaiToOpenAIResponsesRequest(model: string, body: ProtocolRecord, stream: boolean, credentials: ProtocolCredentials) {
    // Body already in Responses API format (e.g. Cursor CLI calling /chat/completions with input[])
    if (body.input) {
        const out: ProtocolRecord = { ...body, model, stream: true };
    ...
    if (body.tools && Array.isArray(body.tools)) {
        result.tools = body.tools.map((tool: ProtocolValue) => {
