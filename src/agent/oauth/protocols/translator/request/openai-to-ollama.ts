 ...
 */
export function openaiToOllamaRequest(model: string, body: ProtocolRecord, stream: boolean) {
 ...
 */
function normalizeMessages(messages: ProtocolValue) {
    ...
    for (const msg of messages) {
        ...
        if (msg.role === ROLE.ASSISTANT && msg.tool_calls) {
            ...
            const ollamaToolCalls = msg.tool_calls.map((tc: ProtocolValue) => ({
...
function normalizeContent(content: ProtocolValue) {
    ...
    if (Array.isArray(content)) {
        ...
        const textParts = content
            .filter((block: ProtocolValue) => block && block.type === OPENAI_BLOCK.TEXT && block.text)
            .map((block: ProtocolValue) => block.text);
 ...
 */
function extractImagesFromContent(content: ProtocolValue) {
    if (!Array.isArray(content))
        return [];
    const images: ProtocolValue[] = [];
    for (const block of content) {
        if (!block || block.type !== OPENAI_BLOCK.IMAGE_URL)
            continue;
