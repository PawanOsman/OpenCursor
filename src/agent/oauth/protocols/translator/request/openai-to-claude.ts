...
export function openaiToClaudeRequest(model: string, body: ProtocolRecord, stream: boolean) {
    ...
    if (body.messages && Array.isArray(body.messages)) {
        ...
        for (const msg of nonSystemMessages) {
            ...
            const hasToolUse = blocks.some((b: ProtocolValue) => b.type === CLAUDE_BLOCK.TOOL_USE);
            const hasToolResult = blocks.some((b: ProtocolValue) => b.type === CLAUDE_BLOCK.TOOL_RESULT);
            // Separate tool_result from other content
            if (hasToolResult) {
                const toolResultBlocks = blocks.filter((b: ProtocolValue) => b.type === CLAUDE_BLOCK.TOOL_RESULT);
                const otherBlocks = blocks.filter((b: ProtocolValue) => b.type !== CLAUDE_BLOCK.TOOL_RESULT);
                flushCurrentMessage();
        ...
        for (let i = result.messages.length - 1; i >= 0; i--) {
            ...
            if (message.role === ROLE.ASSISTANT && Array.isArray(message.content) && message.content.length > 0) {
                ...
                const validBlockTypes: ProtocolValue[] = [CLAUDE_BLOCK.TEXT, CLAUDE_BLOCK.TOOL_USE, CLAUDE_BLOCK.TOOL_RESULT, CLAUDE_BLOCK.IMAGE];
...
Respond ONLY with the JSON object, no other text.`);
    ...
    const claudeCodePrompt: ProtocolRecord = { type: CLAUDE_BLOCK.TEXT, text: CLAUDE_SYSTEM_PROMPT };
...
function getContentBlocksFromMessage(msg: ProtocolValue, toolNameMap: ProtocolValue = new Map()) {
...
function convertOpenAIToolChoice(choice: ProtocolValue) {
    if (!choice)
        return { type: "auto" };
...
function openaiToClaudeRequestForAntigravity(model: string, body: ProtocolRecord, stream: boolean) {
    ...
    if (result.system && Array.isArray(result.system)) {
        result.system = result.system.filter((block: ProtocolValue) => !block.text || !block.text.includes("You are Claude Code"));
    ...
    if (result.tools && Array.isArray(result.tools)) {
        result.tools = result.tools.map((tool: ProtocolValue) => {
    ...
    if (result.messages && Array.isArray(result.messages)) {
        result.messages = result.messages.map((msg: ProtocolValue) => {
            if (!msg.content || !Array.isArray(msg.content)) {
                return msg;
            }
            const updatedContent = msg.content.map((block: ProtocolValue) => {
