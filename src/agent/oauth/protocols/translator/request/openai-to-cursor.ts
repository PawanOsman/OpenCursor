...
function extractContent(content: ProtocolValue) {
    if (typeof content === "string")
        return content;
    if (Array.isArray(content)) {
        return content
            .filter((part: ProtocolValue) => {
            if (!part || typeof part !== "object")
                return false;
            return part.type === OPENAI_BLOCK.TEXT && typeof part.text === "string";
        })
            .map((part: ProtocolValue) => part.text || "")
            .join("");
    }
    return "";
}
function sanitizeToolResultText(text: ProtocolValue) {
    // Strip non-printable control chars that can produce backend request errors
    return text.replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g, "");
}
function escapeXml(text: ProtocolValue) {
    return text.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
}
function buildToolResultBlock(toolName: string, toolCallId: ProtocolValue, resultText: ProtocolValue) {
...
function normalizeToolCallId(id: ProtocolValue) {
    return typeof id === "string" ? id.split("\n")[0] : "";
}
function convertMessages(messages: ProtocolValue) {
    const result: ProtocolValue[] = [];
    // Build a map of tool_call_id -> tool name from assistant tool calls
    const toolCallMetaMap = new Map();
    const rememberToolMeta = (toolCallId: ProtocolValue, toolName: ProtocolValue) => {
        if (!toolCallId)
            return;
    ...
    for (const msg of messages) {
        ...
        if (msg.role === ROLE.ASSISTANT && Array.isArray(msg.content)) {
            for (const part of msg.content) {
                if (part?.type !== CLAUDE_BLOCK.TOOL_USE)
                    continue;
    ...
    for (let i = 0; i < messages.length; i++) {
        ...
        if (msg.role === ROLE.USER || msg.role === ROLE.ASSISTANT) {
            ...
            if (msg.role === ROLE.ASSISTANT && msg.tool_calls && msg.tool_calls.length > 0) {
                const assistantMsg: ProtocolRecord = { role: ROLE.ASSISTANT, content: content || "" };
                assistantMsg.tool_calls = msg.tool_calls.map((tc: ProtocolValue) => {
                    const { index, ...rest } = tc || {};
                    return rest;
                });
                ...
                const extractedToolCalls = msg.content
                    .filter((b: ProtocolValue) => b?.type === CLAUDE_BLOCK.TOOL_USE)
                    .map((b: ProtocolValue) => ({
                    id: b.id || "",
...
export function openaiToCursorRequest(model: string, body: ProtocolRecord, stream: boolean, credentials: ProtocolCredentials) {
