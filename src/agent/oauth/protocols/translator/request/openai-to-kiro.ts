...
import type { ProtocolValue, ProtocolRecord, ProtocolCredentials } from "../../wireTypes.js";
/**
...
import { resolveKiroModelIntent, applyKiroThinkingOverride, resolveKiroThinkingBudget, buildThinkingSystemPrefix, KIRO_AGENTIC_SYSTEM_PROMPT, resolveDefaultProfileArn, buildKiroAdditionalModelRequestFieldsForModel, usesKiroNativeGptEffort } from "../../config/kiroConstants.js";
import { parseDataUri } from "../concerns/image.js";
import { DEFAULT_IMAGE_MIME } from "../schema/index.js";
import { ROLE, OPENAI_BLOCK, CLAUDE_BLOCK } from "../schema/index.js";
import { canonicalizeKiroConversation, normalizeKiroToolSpecs, kiroEmptyUserContent, } from "../concerns/kiroConversation.js";
/**
 * Safely parse JSON string, returning fallback on failure.
 */
function safeJSONParse(str: ProtocolValue, fallback: ProtocolValue) {
 ...
 */
function convertMessages(messages: ProtocolValue, model: string) {
    let history: ProtocolValue[] = [];
    let currentMessage: ProtocolValue = null;
    let pendingUserContent: ProtocolValue[] = [];
    let pendingAssistantContent: ProtocolValue[] = [];
    let pendingToolResults: ProtocolValue[] = [];
    let pendingImages: ProtocolValue[] = [];
    let currentRole: ProtocolValue = null;
    const flushPending = () => {
        if (currentRole === "user") {
            const content = pendingUserContent.join("\n\n").trim()
                || kiroEmptyUserContent(pendingToolResults.length > 0);
            const userMsg: ProtocolRecord = {
                userInputMessage: {
                    content: content,
                    modelId: ""
                }
            ...
            const assistantMsg: ProtocolRecord = {
    ...
    for (let i = 0; i < messages.length; i++) {
        ...
        if (role === ROLE.USER) {
            ...
            if (typeof msg.content === "string") {
                ...
                const toolResultBlocks = msg.content.filter((c: ProtocolValue) => c.type === CLAUDE_BLOCK.TOOL_RESULT);
                if (toolResultBlocks.length > 0) {
                    toolResultBlocks.forEach((block: ProtocolValue) => {
                        const text = Array.isArray(block.content)
                            ? block.content.map((c: ProtocolValue) => c.text || "").join("\n")
            ...
            let textContent = "";
            let toolUses: ProtocolValue[] = [];
            if (Array.isArray(msg.content)) {
                const textBlocks = msg.content.filter((c: ProtocolValue) => c.type === OPENAI_BLOCK.TEXT);
                textContent = textBlocks.map((b: ProtocolValue) => b.text).join("\n").trim();
                const toolUseBlocks = msg.content.filter((c: ProtocolValue) => c.type === CLAUDE_BLOCK.TOOL_USE);
            ...
            if (toolUses.length > 0) {
                ...
                if (lastMsg?.assistantResponseMessage) {
                    lastMsg.assistantResponseMessage.toolUses = toolUses.map((tc: ProtocolValue) => {
                        if (tc.function) {
                            return {
    ...
    history.forEach((item: ProtocolValue) => {
    ...
    const mergedHistory: ProtocolValue[] = [];
 ...
 *
 * Two OpenCursor-specific behaviours implemented here:
 *
 ...
 */
export function openaiToKiroRequest(model: string, body: ProtocolRecord, stream: boolean, credentials: ProtocolCredentials) {
    ...
    const systemPromptParts: ProtocolValue[] = [];
    ...
    for (const [original, sanitized] of nameMap) {
        if (original !== sanitized)
            restoredToolNames.set(sanitized, original);
    }
    if (restoredToolNames.size)
        payload._toolNameMap = restoredToolNames;
    return payload;
}
