 ...
 * on the finish chunk or drop `choices: []` entirely. OpenCursor's own dashboard
 ...
 */
export function canonicalizeQoderUsage(usage: ProtocolValue) {
    ...
    const cacheCreation = num(details.cache_creation_tokens ??
...
function finishReasonOf(parsed: ProtocolValue) {
    const choice = parsed?.choices?.[0];
    return choice?.finish_reason || choice?.delta?.finish_reason || parsed?.finish_reason || null;
}
function hasValuableDelta(parsed: ProtocolValue) {
...
function parseInner(inner: ProtocolValue) {
    if (inner == null || inner === "")
        return { raw: false, parsed: null };
    if (inner === "[DONE]")
        return { done: true };
 ...
 */
export function createQoderSseCoalescer({ model, encoder, sseDone }: ProtocolValue) {
    let pendingFinish: ProtocolValue = null;
    let pendingUsage: ProtocolValue = null;
    let lastMeta: ProtocolRecord = { id: null, created: null, model };
    let doneEmitted = false;
    let finishAlreadyForwarded = false;
    const emitJson = (controller: ProtocolValue, obj: ProtocolValue) => {
        const sanitized = JSON.stringify(obj).replace(/\r?\n/g, "");
        controller.enqueue(encoder.encode(`data: ${sanitized}\n\n`));
    };
    const emitRaw = (controller: ProtocolValue, text: ProtocolValue) => {
        controller.enqueue(encoder.encode(`data: ${String(text).replace(/\r?\n/g, "")}\n\n`));
    };
    const emitDone = (controller: ProtocolValue) => {
        if (doneEmitted)
            return;
        controller.enqueue(encoder.encode(sseDone));
        doneEmitted = true;
    };
    const emitTerminal = (controller: ProtocolValue) => {
    ...
    const flush = (controller: ProtocolValue) => {
        if (doneEmitted)
            return;
    ...
    const handleInner = (inner: ProtocolValue, controller: ProtocolValue) => {
        if (doneEmitted)
        ...
        if (typeof parsed.id === "string" && parsed.id)
            lastMeta.id = parsed.id;
        if (typeof parsed.created === "number")
            lastMeta.created = parsed.created;
        if (typeof parsed.model === "string" && parsed.model)
            lastMeta.model = parsed.model;
