...
import type { ProtocolRecord, ProtocolCredentials } from "../../wireTypes.js";
/**
 * Qoder API endpoints and protocol constants.
 *
...
const QODER_REGION_BASES: ProtocolRecord = {
    [QODER_REGION_INTL]: {
...
export function qoderRegionBases(region: string) {
...
export function qoderRegionOf(providerId: string) {
...
export function qoderOpenApiBase(region: string) {
    return qoderRegionBases(region).openApi;
}
export function qoderDeviceTokenUrl(region: string) {
    return `${qoderOpenApiBase(region)}/api/v1/deviceToken/poll`;
}
export function qoderUserInfoUrl(region: string) {
    return `${qoderOpenApiBase(region)}/api/v1/userinfo`;
}
export function qoderQuotaUsageUrl(region: string) {
    return `${qoderOpenApiBase(region)}/api/v2/quota/usage`;
}
export function qoderRefreshTokenUrl(region: string) {
    return `${qoderRegionBases(region).center}/algo/api/v3/user/refresh_token`;
}
export function qoderLoginUrl(region: string) {
    return qoderRegionBases(region).login;
}
export function qoderWebsiteUrl(region: string) {
...
export function qoderJobTokenExchangeUrl(region: string) {
...
export function qoderModelListUrl(region: string) {
 ...
 */
export function qoderInferenceBase(credentials: ProtocolCredentials, region: string = QODER_REGION_INTL) {
    // CN serves every token kind from the single gateway host.
    if (region === QODER_REGION_CN)
        return QODER_REGION_BASES[QODER_REGION_CN].chat;
...
// Client metadata used in provider requests.
