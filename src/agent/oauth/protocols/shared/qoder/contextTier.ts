 ...
 * picker; a qodercli-style client (which is what OpenCursor impersonates) has no picker,
...
const UNIT: Record<string, number> = { K: 1000, M: 1000000 };
/** "200K" | "1M" | "204800" | 204800 → integer token count (0 when unparseable). */
export function parseTierTokenCount(value: ProtocolValue) {
...
function tierName(entry: ProtocolValue, tokenCount: ProtocolValue) {
    const raw = entry.name ?? entry.label ?? entry.display_name ?? entry.displayName ?? entry.key ?? entry.id;
    if (typeof raw === "string" && raw.trim())
        return raw.trim();
    if (tokenCount >= UNIT.M && tokenCount % UNIT.M === 0)
        return `${tokenCount / UNIT.M}M`;
    if (tokenCount >= UNIT.K && tokenCount % UNIT.K === 0)
        return `${tokenCount / UNIT.K}K`;
 ...
 */
export function getQoderContextTiers(modelConfig: ProtocolValue) {
    ...
    for (const entry of list) {
        if (!entry || typeof entry !== "object")
            continue;
        const tokenCount = parseTierTokenCount(entry.tokenCount ?? entry.token_count ?? entry.max_input_tokens ?? entry.maxInputTokens ?? entry.contextLength ?? entry.context_length);
        if (!tokenCount)
            continue;
    ...
    return [...byCount.values()].sort((a: ProtocolValue, b: ProtocolValue) => a.tokenCount - b.tokenCount);
}
 ...
 */
export function estimateQoderPromptTokens({ system, messages, tools }: ProtocolValue = {}) {
    let text = "";
    try {
...
function normalizeMode(preference: ProtocolValue) {
    const p = String(preference ?? "").trim();
    return p ? p : QODER_CONTEXT_TIER_MODES.AUTO;
}
function findNamedTier(tiers: ProtocolValue, name: ProtocolValue) {
    const wanted = name.replace(/\s+/g, "").toUpperCase();
    const asCount = parseTierTokenCount(wanted);
    return tiers.find((t: ProtocolValue) => t.name.replace(/\s+/g, "").toUpperCase() === wanted || (asCount && t.tokenCount === asCount)) || null;
}
/**
 ...
 */
export function resolveQoderContextTier(modelConfig: ProtocolValue, prompt: ProtocolValue, options: ProtocolRecord = {}) {
    ...
    const defaultTier = tiers.find((t: ProtocolValue) => t.isDefault) || tiers[0];
    ...
    if (mode.toLowerCase() !== QODER_CONTEXT_TIER_MODES.AUTO) {
        ...
        if (named)
            return { tier: named, estimatedTokens, reason: `forced:${named.name}` };
    ...
    if (need <= currentLimit)
        return null;
    const fits = tiers.find((t: ProtocolValue) => t.tokenCount >= need && t.tokenCount > currentLimit);
    const tier = fits || largest;
    if (tier.tokenCount <= currentLimit)
        return null; // nothing bigger to escalate to
 ...
 */
export function applyQoderContextTier(payload: ProtocolRecord, tier: ProtocolValue) {
    if (!payload || !tier?.tokenCount)
        return payload;
