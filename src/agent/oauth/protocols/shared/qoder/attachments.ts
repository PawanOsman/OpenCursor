 ...
 * data-URIs instead, which OpenCursor previously forwarded verbatim — 10MB
...
import { QODER_IMAGE_UPLOAD_SIG_PATH, QODER_INLINE_FALLBACK_MAX_BYTES, QODER_MAX_PAYLOAD_BYTES, qoderInferenceBase, } from "./constants.js";
const IMAGE_MIME_RE = /^image\//i;
const DATA_URI_RE = /data:[^;]+;base64,[A-Za-z0-9+/=\s]+/g;
function mimeExt(mime: ProtocolValue) {
...
function decodedBytes(b64: ProtocolValue) {
    if (typeof b64 !== "string" || !b64)
        return 0;
    const compact = b64.replace(/\s/g, "");
    return Math.floor(compact.length * 3 / 4);
}
function stubText({ name, mime, bytes, reason }: ProtocolValue) {
    const label = name || mime || "attachment";
    const size = bytes ? `, ${bytes} bytes` : "";
    return `[file omitted: ${label}${size} — ${reason}]`;
}
export function buildMultipartFile(buffer: ProtocolValue, { fieldName = "file", fileName, mediaType }: ProtocolValue = {}) {
    const boundary = `----OpenCursorQoder${Date.now().toString(16)}${Math.random().toString(16).slice(2)}`;
...
function extractUrlFromUploadResponse(json: ProtocolValue) {
    if (!json || typeof json !== "object")
        return null;
    const result = json.result && typeof json.result === "object" ? json.result : json;
    const arrays: ProtocolValue[] = [result.imageUrls, result.image_urls, json.imageUrls, json.image_urls];
    for (const arr of arrays) {
        if (Array.isArray(arr) && typeof arr[0] === "string" && arr[0])
            return arr[0];
    }
    const keys: string[] = ["imageUrl", "image_url", "url", "ossUrl", "oss_url", "originalUrl", "originUrl", "link", "image"];
...
async function defaultUploadImage({ buffer, mediaType, credentials, proxyOptions, signal }: ProtocolValue) {
    ...
    const json: ProtocolValue = await res.json().catch(() => null);
    const uploaded = extractUrlFromUploadResponse(json);
    if (!uploaded)
        throw new Error("upload response missing url");
    return uploaded;
}
async function uploadImageData({ base64, mediaType, credentials, proxyOptions, signal, log, uploadFn, cache }: ProtocolValue) {
    ...
    if (cache?.has(digest))
        return { url: cache.get(digest), bytes, mime: mediaType };
    ...
    if (bytes <= QODER_INLINE_FALLBACK_MAX_BYTES)
        return { keep: true, bytes, mime: mediaType };
    return { stub: true, bytes, mime: mediaType };
}
function imageUrlBlock(url: ProtocolValue) {
    return { type: OPENAI_BLOCK.IMAGE_URL, image_url: { url } };
}
async function rewriteBlock(block: ProtocolValue, ctx: ProtocolValue) {
    ...
    if (block.type === OPENAI_BLOCK.IMAGE_URL) {
        ...
        if (raw.startsWith("http://") || raw.startsWith("https://"))
            return imageUrlBlock(raw);
        const parsed = parseDataUri(raw);
        if (!parsed)
            return { type: OPENAI_BLOCK.TEXT, text: stubText({ name: "attachment", reason: "unreadable data URI" }) };
    ...
    if (block.type === OPENAI_BLOCK.IMAGE || block.type === CLAUDE_BLOCK.IMAGE) {
        ...
        if (src.type === "base64" && src.data) {
            ...
            if (up?.url)
                return imageUrlBlock(up.url);
            if (up?.keep)
                return imageUrlBlock(`data:${mime};base64,${src.data}`);
    ...
    if (typeof block.text === "string" && block.text.includes("data:") && block.text.length > 8192) {
        const next = block.text.replace(DATA_URI_RE, (m: ProtocolValue) => {
...
async function rewriteContent(content: ProtocolValue, ctx: ProtocolValue) {
    if (typeof content === "string") {
        if (content.includes("data:") && content.length > 8192) {
            return content.replace(DATA_URI_RE, (m: ProtocolValue) => {
                const parsed = parseDataUri(m.trim());
                const bytes = parsed ? decodedBytes(parsed.base64) : m.length;
                if (bytes <= QODER_INLINE_FALLBACK_MAX_BYTES)
                    return m;
...
function payloadBytes(messages: ProtocolValue) {
    try {
...
function stripRemainingDataUris(messages: ProtocolValue) {
    for (const msg of messages || []) {
        if (typeof msg?.content === "string" && msg.content.includes("data:")) {
            msg.content = msg.content.replace(DATA_URI_RE, (m: ProtocolValue) => stubText({ bytes: m.length, reason: "payload over Qoder size budget" }));
        }
        else if (Array.isArray(msg?.content)) {
            msg.content = msg.content.map((block: ProtocolValue) => {
                if (block?.type === OPENAI_BLOCK.IMAGE_URL) {
                    const raw = typeof block.image_url === "string" ? block.image_url : block.image_url?.url;
                ...
                if (typeof block?.text === "string" && block.text.includes("data:")) {
                    return { ...block, text: block.text.replace(DATA_URI_RE, (m: ProtocolValue) => stubText({ bytes: m.length, reason: "payload over Qoder size budget" })) };
                }
                return block;
            });
        }
 ...
 */
export async function rewriteQoderMessageAttachments(messages: ProtocolValue, { credentials, log, proxyOptions = null, signal = null, uploadFn = null, }: ProtocolValue = {}) {
    const stats: ProtocolRecord = { imageUrls: [], uploaded: 0, stubbed: 0 };
    if (!Array.isArray(messages) || messages.length === 0)
        return stats;
    const ctx: ProtocolRecord = { credentials, log, proxyOptions, signal, uploadFn, cache: new Map() };
    for (const msg of messages) {
        if (!msg || typeof msg !== "object")
            continue;
        if (Array.isArray(msg.images)) {
            // Ollama-style sidecar; fold into content so normalizeMessages can see them.
            const extras = msg.images.map((url: ProtocolValue) => imageUrlBlock(String(url)));
    ...
    for (const msg of messages) {
        ...
        for (const block of msg.content) {
            ...
            if (typeof url === "string" && /^https?:\/\//i.test(url))
                stats.imageUrls.push(url);
            if (block?.type === OPENAI_BLOCK.TEXT && typeof block.text === "string" && block.text.startsWith("[file omitted:"))
                stats.stubbed += 1;
        }
    }
...
export const __test__: ProtocolRecord = {
