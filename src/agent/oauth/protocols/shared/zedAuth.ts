...
export const ZED_HEADERS: Record<string, string> = {
...
function b64url(value: ProtocolValue) {
    return Buffer.from(value).toString("base64url");
}
function b64urlPadded(buf: ProtocolValue) {
    return buf.toString("base64").replace(/\+/g, "-").replace(/\//g, "_");
}
function fromB64url(value: ProtocolValue) {
    return Buffer.from(String(value || ""), "base64url").toString("utf8");
}
function normalizeBaseUrl(baseUrl: ProtocolValue, fallback: ProtocolValue) {
    return String(baseUrl || fallback).replace(/\/+$/, "");
}
function zedUrl(config: ProtocolRecord, key: ProtocolValue, path: ProtocolValue, fallbackBase: ProtocolValue) {
...
export function encodeZedPrivateKeyVerifier(privateKeyPem: ProtocolValue) {
    return `${PRIVATE_KEY_PREFIX}${b64url(privateKeyPem)}`;
}
export function decodeZedPrivateKeyVerifier(verifier: ProtocolValue) {
...
export function createZedNativeAuthData(config: ProtocolRecord = {}, options: ProtocolRecord = {}) {
...
export function parseZedCallbackPayload(input: ProtocolValue) {
    const raw = String(input || "").trim();
    if (!raw)
        throw new Error("Missing Zed callback URL");
    let data: ProtocolRecord = {};
    try {
        data = JSON.parse(raw);
    }
    catch {
        let url: ProtocolValue;
        try {
            url = new URL(raw);
        }
        catch {
        ...
        url.searchParams.forEach((value: ProtocolValue, key: ProtocolValue) => {
            data[key] = value;
        });
    }
...
export function decryptZedAccessToken(encryptedAccessToken: ProtocolValue, privateKeyVerifier: ProtocolValue) {
    ...
    const fail = (oaepError: ProtocolValue) => {
    ...
    try {
        return crypto
            .privateDecrypt({ key: privateKey, padding: crypto.constants.RSA_PKCS1_OAEP_PADDING, oaepHash: "sha256" }, encrypted)
            .toString("utf8");
    }
    catch (caughtError) {
        const oaepError = caughtError as ProtocolError;
        try {
            const text = crypto
            ...
            if (err.message.startsWith("Failed to decrypt Zed access token"))
                throw err;
            fail(oaepError);
        }
    }
}
export function buildZedUserAuthHeader(credentials: ProtocolCredentials) {
...
function getSystemId(credentials: ProtocolCredentials) {
    return String(credentials?.providerSpecificData?.systemId || credentials?.systemId || "");
}
async function fetchJson(url: ProtocolValue, options: ProtocolRecord, proxyOptions: ProtocolValue = null) {
    const res = await proxyAwareFetch(url, options, proxyOptions);
    const text = await res.text();
    let data: ProtocolValue = null;
    if (text) {
        try {
    ...
    if (!res.ok) {
        ...
        const err: ProtocolError = new Error(String(message));
...
export async function fetchZedAuthenticatedUser(credentials: ProtocolCredentials, options: ProtocolRecord = {}) {
    ...
    if (systemId)
        headers[ZED_HEADERS.systemId] = systemId;
...
function normalizeOrganizationId(value: ProtocolValue) {
    if (!value)
        return "";
...
export function resolveZedOrganizationId(credentials: ProtocolCredentials, userInfo: ProtocolValue = null) {
    const psd = credentials?.providerSpecificData || {};
    ...
    const org = orgs.find((item: ProtocolValue) => item?.is_personal) || orgs[0];
    return normalizeOrganizationId(org?.id);
}
function zedUserCacheKey(credentials: ProtocolCredentials, organizationId: ProtocolValue) {
    const psd = credentials?.providerSpecificData || {};
    const userId = psd.userId || credentials?.userId || "unknown";
    const token = credentials?.accessToken || credentials?.apiKey || "";
    return `${userId}:${organizationId || "default"}:${token.slice(-16)}`;
}
function zedModelCacheKey(credentials: ProtocolCredentials) {
    const psd = credentials?.providerSpecificData || {};
    const org = psd.organizationId || psd.defaultOrganizationId || "default";
    const token = credentials?.accessToken || credentials?.apiKey || "";
    return `${psd.userId || "unknown"}:${org}:${token.slice(-16)}`;
}
export async function fetchZedLlmToken(credentials: ProtocolCredentials, options: ProtocolRecord = {}) {
    const config = options.config || {};
    ...
    if (!organizationId)
        throw new Error("No Zed organization selected");
    ...
    const token = typeof data?.token === "string" ? data.token : data?.token?.[0] || data?.token?.value;
    if (!token)
        throw new Error("Zed did not return an LLM token");
    llmTokenCache.set(cacheKey, { token, expiresAt: Date.now() + LLM_TOKEN_TTL_MS });
    return token;
}
export function shouldRefreshZedLlmToken(response: ProtocolValue) {
    return (response?.status === 401 ||
        !!response?.headers?.has?.(ZED_HEADERS.expiredToken) ||
        !!response?.headers?.has?.(ZED_HEADERS.outdatedToken));
}
export async function zedLlmFetch(credentials: ProtocolCredentials, path: ProtocolValue, options: ProtocolRecord = {}) {
    const config = options.config || {};
    const url = zedUrl(config, "llmBaseUrl", path, ZED_LLM_BASE_URL);
    const buildRequest = async (forceRefresh: ProtocolValue) => {
...
function normalizeZedModelId(id: ProtocolValue) {
    if (!id)
        return "";
    if (typeof id === "string")
        return id;
...
export function mapZedModel(model: ProtocolRecord) {
...
export async function resolveZedModels(credentials: ProtocolCredentials, options: ProtocolRecord = {}) {
    ...
    if (existing && !options.forceRefresh)
        return existing;
    const promise: ProtocolValue = (async () => {
        ...
        const models = rawModels
            .map(mapZedModel)
            .filter(Boolean)
            .filter((model: ProtocolValue) => !model.isDisabled);
    ...
    try {
        ...
        if (modelInflight.get(key) === promise)
