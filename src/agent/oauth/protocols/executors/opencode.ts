...
function hasValidOpencodeVersion(ua: ProtocolValue) {
...
export function generateSessionId(timestamp = Date.now()) {
  ...
  const value = ~current;
  const time = Array.from({ length: 6 }, (_: ProtocolValue, index: number) =>
...
export function translateSessionId(sessionId: string, clientTool = "") {
...
function normalizeSession(value: ProtocolValue) {
...
function nativeSession(headers: Record<string, string>) {
...
function identityKey(credentials: ProtocolCredentials) {
  ...
  const raw: Record<string, string> = credentials?.rawHeaders || {};
...
export function stableSessionId(credentials: ProtocolCredentials) {
...
function lastUserText(body: ProtocolRecord | undefined) {
  try {
    ...
    for (let i = arr.length - 1; i >= 0; i--) {
      ...
      if (Array.isArray(content)) {
        const text = content
          .map((part: ProtocolRecord) => (typeof part === "string" ? part : part?.text || part?.input_text || ""))
          .join(" ")
          .trim();
...
export function deriveRequestId(sessionId: string, body: ProtocolRecord | undefined) {
...
function normalizeRequestId(value: ProtocolValue) {
  if (typeof value !== "string") return null;
  const normalized = value.trim();
  if (!normalized || normalized.length > MAX_SESSION_LENGTH) return null;
...
function bodyHasSessionHints(body: ProtocolRecord | undefined) {
  try {
...
function baseModelId(model: string) {
...
function isMessagesModel(model: string) {
  return MESSAGES_MODELS.has(baseModelId(model));
}

function resolveOpencodeSession(body: ProtocolRecord | undefined, credentials: ProtocolCredentials, providerSessionId: ProtocolValue, clientTool: ProtocolValue) {
  const headers = credentials?.rawHeaders || {};
  const native = nativeSession(headers);
  if (native) return native;

  let incoming: ProtocolValue = null;
  for (const [key, value] of Object.entries(headers)) {
    if (key.toLowerCase() === SESSION_HEADER) {
  ...
  if (credentials?.connectionId || bodyHasSessionHints(body)) {
    let viaManager: ProtocolValue = null;
    try {
...
function resolveOpencodeRequestId(body: ProtocolRecord | undefined, credentials: ProtocolCredentials, sessionId: string) {
  const raw: Record<string, string> = credentials?.rawHeaders || {};
...
function normalizeResponsesTools(body: ProtocolRecord) {
  if (!Array.isArray(body.tools)) return;
  const validNames = new Set();
  body.tools = body.tools.filter((tool: ProtocolRecord) => {
...
function sanitizeResponsesItems(body: ProtocolRecord) {
  if (!Array.isArray(body.input)) return;
  body.input = body.input.filter((item: ProtocolValue) => {
...
function normalizeOpencodeReasoning(model: string, body: ProtocolRecord) {
...
export class OpenCodeExecutor extends BaseExecutor {
  ...
  prepareRequestCredentials({ body, credentials, providerSessionId, clientTool }: { body?: ProtocolRecord; credentials?: ProtocolCredentials; providerSessionId?: string; clientTool?: string } = {}) {
  ...
  transformRequest(model: string, body: ProtocolRecord, stream = true, credentials: ProtocolCredentials = {}) {
