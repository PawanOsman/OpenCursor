...
function toolNameOf(tool: ProtocolRecord) {
...
function ensureChatFingerprintTools(body: ProtocolRecord) {
...
function ensureResponsesFingerprintTools(body: ProtocolRecord) {
  if (!body || typeof body !== "object") return;
  const present = new Set();
  if (Array.isArray(body.tools)) {
    for (const tool of body.tools) {
...
function translatedSession(sessionId: string, clientTool: ProtocolValue) {
...
export class OpenCodeZenExecutor extends DefaultExecutor {
  ...
  transformRequest(model: string, body: ProtocolRecord, stream = true, credentials: ProtocolCredentials = {}) {
    ...
    if (!isResponsesModel(model || body?.model)) {
      /* Only caller-declared tools are sent. */
      return out;
    }
