...
import type { ProtocolRecord, ProtocolValue, ProtocolCredentials, ProtocolError, TranslatorState, ResponseChunk } from "../wireTypes.js";
...
// the RSA-decrypted access_token (see ../shared/zedAuth.ts). The
...
function normalizeZedProvider(value: ProtocolValue, model: string) {
...
function buildProviderRequest(provider: string, model: string, body: ProtocolRecord, stream: boolean, credentials: ProtocolCredentials) {
...
function initProviderState(provider: string, model: string): TranslatorState {
...
function convertProviderEvent(provider: string, event: ProtocolRecord | null, state: TranslatorState): ResponseChunk | ResponseChunk[] | null {
...
function enqueueSseObject(controller: TransformStreamDefaultController<Uint8Array>, encoder: InstanceType<typeof TextEncoder>, chunk: ResponseChunk | ResponseChunk[] | null) {
  if (!chunk) return;
...
function normalizeStatus(status: unknown): ProtocolRecord | null {
  if (!status) return null;
  if (typeof status === "string") return { type: status };
  if (typeof status === "object") {
    const record = status as ProtocolRecord;
    const key = Object.keys(record)[0];
    if (key && typeof record[key] === "object") return { type: key, ...record[key] };
    return record;
  }
  return null;
}
...
class ZedExecutor extends BaseExecutor {
  ...
  async resolveModel(model: string, credentials: ProtocolCredentials, signal: AbortSignal | undefined, log: ExecutorLogger | undefined) {
    try {
