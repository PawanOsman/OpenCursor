...
const GROK_CLI_FREEFORM_TOOL_PARAMETERS: ProtocolRecord = {
  type: "object",
 ...
 */
export function countGrokCliUserTurns(input: ProtocolRecord) {
 ...
 */
export function resolveGrokCliTurnIdx(sessionId: string, input: ProtocolRecord, requestKey: object | null = null) {
...
export function normalizeGrokCliEffort(value: ProtocolValue) {
...
export function resolveGrokCliSessionId(credentials: ProtocolCredentials, body: ProtocolRecord) {
  ...
  const explicitSessionBody: ProtocolRecord = {
...
function stringifyGrokCliToolOutput(output: ProtocolValue) {
  if (typeof output === "string") return output;
  if (output === undefined) return "";
  return JSON.stringify(output);
}

function isNativeGrokCliItemId(id: string) {
  return typeof id === "string" && GROK_CLI_NATIVE_ITEM_ID.test(id);
}

function normalizeGrokCliInputItem(item: ProtocolValue) {
...
export function normalizeGrokCliInput(body: ProtocolRecord) {
  if (!Array.isArray(body?.input)) return body;
  const normalized = body.input.map(normalizeGrokCliInputItem).filter(Boolean);
  const callIds = new Set(
    normalized
      .filter((item: ProtocolValue) => item?.type === "function_call" && item.call_id)
      .map((item: ProtocolValue) => item.call_id)
  );
  body.input = normalized.filter(
    (item: ProtocolValue) => item?.type !== "function_call_output" || callIds.has(item.call_id)
  );
  return body;
}

function stripStoredItemReferences(body: ProtocolRecord) {
 ...
 */
function normalizeGrokCliTools(body: ProtocolRecord) {
...
function resolveEffortFromModel(modelId: string) {
...
export class GrokCliExecutor extends BaseExecutor {
  _currentSessionId!: ProtocolValue;
  _currentReqId!: ProtocolValue;
  _currentTurnIdx!: ProtocolValue;
  _agentId!: ProtocolValue;
  ...
  transformRequest(model: string, body: ProtocolRecord, stream: boolean, credentials: ProtocolCredentials) {
    ...
    if (!body.input || (Array.isArray(body.input) && body.input.length === 0)) {
      if (Array.isArray(body.messages) && body.messages.length > 0) {
        ...
        body.input = body.messages.map((m: ProtocolRecord) => ({
          type: "message",
          role: m.role || "user",
