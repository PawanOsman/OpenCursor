...
function flattenQuery(messages: ProtocolRecord[]) {
  const parts: string[] = [];
...
export default class TraeExecutor extends BaseExecutor {
  ...
  commonParams(psd: ProtocolRecord, mode: string, sessionId?: string) {
    const cp: ProtocolRecord = {
      language: "en-us",
  ...
  async createSession(headers: Record<string, string>, query: string, model: string, psd: ProtocolRecord, signal: AbortSignal | undefined) {
    const { mode, strategy, modelName } = this.resolveMode(model);
    const body: ProtocolRecord = {
      mode,
  ...
  async streamEvents(headers: Record<string, string>, sessionId: string, replyTo: string, onEvent: (event: string | null, data: ProtocolRecord) => boolean, signal: AbortSignal | undefined) {
  ...
  async execute({ model, body, stream, credentials, signal }: ExecutorRequest): Promise<ExecutorResult> {
...
const err = caught as ProtocolError;
    ...
    const order: ProtocolValue[] = [];
    const thoughts: ProtocolRecord = {};
    let sent = 0;
    let usage: ProtocolValue = null;
    let errorEvent: ProtocolValue = null;
    const renderNewText = (data: ProtocolRecord) => {
      const pid = data.id;
      if (!pid) return "";
      if (!(pid in thoughts)) order.push(pid);
      const t = data.thought || "";
      if (t.length >= (thoughts[pid] || "").length) thoughts[pid] = t;
      const full = order.map((i: ProtocolValue) => thoughts[i]).join("");
    ...
    if (stream !== false) {
      ...
      const sse = new ReadableStream({
        start: async (controller: ReadableStreamDefaultController<Uint8Array>) => {
          const emit = (obj: ProtocolValue) => controller.enqueue(enc.encode(`data: ${JSON.stringify(obj)}\n\n`));
          emit({
            id: responseId,
          ...
          try {
            await this.streamEvents(headers, session.sessionId, session.messageId, (ev: ProtocolValue, data: ProtocolRecord) => {
...
const err = caught as ProtocolError;
    ...
    const content = order.map((i: ProtocolValue) => thoughts[i]).join("");
