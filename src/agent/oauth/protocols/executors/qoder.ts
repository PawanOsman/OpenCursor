 ...
 */
function normalizeMessages(messages: ProtocolRecord[]) {
  ...
  for (const msg of messages) {
    ...
    const cloned: ProtocolRecord = { ...msg };
 ...
 */
function normalizeContent(content: ProtocolValue) {
  if (typeof content === "string") return content;
  if (content == null) return "";
  if (!Array.isArray(content)) return String(content);

  const blocks: ProtocolValue[] = [];
  ...
  const imageUrlOf = (item: ProtocolValue) => {
...
function extractText(content: ProtocolValue) {
  if (typeof content === "string") return content;
  if (content == null) return "";
...
function lastUserText(messages: ProtocolRecord[]) {
...
function stableHash(prefix: ProtocolValue, ...parts: ProtocolValue[]) {
...
function stableChatRecordId(model: string, messages: ProtocolRecord[], tools: ProtocolRecord[], maxTokens: ProtocolValue) {
  const h = createHash("sha256");
...
function truncate(s: ProtocolValue, n: ProtocolValue) {
 ...
 */
async function buildQoderRequestBody({ model, body, credentials, log, proxyOptions, signal, uploadFn = null, region = "intl" }: Omit<ExecutorRequest,"stream"> & {uploadFn?: ((...args: ProtocolValue[])=>Promise<ProtocolRecord>) | null; region?: string}) {
  ...
  const incoming = Array.isArray(body.messages)
    ? body.messages.map((m: ProtocolRecord) => {
      if (!m || typeof m !== "object") return m;
      return {
        ...m,
        content: Array.isArray(m.content)
          ? m.content.map((b: ProtocolValue) => (b && typeof b === "object" ? { ...b } : b))
          : m.content,
      };
    })
    : [];
 ...
 */
function isBillingBlock(inner: ProtocolValue) {
 ...
 */
async function peekFirstQoderFrame(reader: ProtocolValue, decoder: ProtocolValue) {
  let consumed = "";
  let offset = 0;
 ...
 */
async function wrapQoderSSE(response: Response, model: string, log: ExecutorLogger | undefined | null = null) {
  ...
  if (peek.isError) {
    ...
    const status = peek.isBilling
      ...
      : typeof peek.statusVal === "number" && Number.isInteger(peek.statusVal) && peek.statusVal >= HTTP_STATUS.BAD_REQUEST && peek.statusVal <= 599
...
export class QoderExecutor extends BaseExecutor {
  ...
  accountUrl(credentials: ProtocolCredentials) {
...
const err = caught as ProtocolError;
        ...
        return { response: fakeResp, url: this.accountUrl(credentials), headers: {}, transformedBody: body };
      }
    }
