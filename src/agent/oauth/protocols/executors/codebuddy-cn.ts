 ...
 * client sent it, so we force it true here — OpenCursor still re-aggregates the
...
export class CodeBuddyExecutor extends DefaultExecutor {
  ...
  transformRequest(model: string, body: ProtocolRecord, stream: boolean, credentials: ProtocolCredentials) {
    ...
    const flatten = (content: ProtocolValue) =>
      typeof content === "string"
        ? content
        : Array.isArray(content)
          ? content.map((b: ProtocolValue) => (b && typeof b.text === "string" ? b.text : "")).join("\n")
          : "";
    if (Array.isArray(transformed.messages)) {
      transformed.messages = transformed.messages.map((message: ProtocolRecord) => {
    ...
    // OpenAI-style params: reasoning_effort + reasoning_summary:"auto". OpenCursor's
