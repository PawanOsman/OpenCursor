...
export class BaseExecutor {
  computeRetryDelay?: (response: Response, attempt: number, delayMs: number) => number | false | null | Promise<number | false | null>;
  provider!: string;
  config!: ExecutorConfig;
  noAuth!: boolean;

  constructor(provider: string, config: ProtocolRecord) {
  ...
  buildHeaders(credentials: ProtocolCredentials, stream = true, _url?: string, _model?: string, _body?: ProtocolRecord): Record<string, string> {
  ...
  transformRequest(model: string, body: ProtocolRecord, stream = true, credentials: ProtocolCredentials = {}): ProtocolRecord | Uint8Array {
    return body;
  }
  ...
  async refreshCredentials(credentials: ProtocolCredentials, log?: ExecutorLogger, proxyOptions: ProtocolRecord | null = null): Promise<TokenRefreshResult | null> {
    return null;
  }
  ...
  parseError(response: Response, bodyText: string): {status:number;message:string;retryAfter?:string} {
    return { status: response.status, message: bodyText || `HTTP ${response.status}` };
  }

  async execute({ model, body, stream, credentials, signal, log, proxyOptions = null }: ExecutorRequest): Promise<ExecutorResult> {
    const fallbackCount = this.getFallbackCount();
    let lastError: ProtocolValue = null;
    let lastStatus = 0;
    const retryAttemptsByUrl: Record<number,number> = {};

    // Merge default retry config with provider-specific config
    const retryConfig: ProtocolRecord = { ...DEFAULT_RETRY_CONFIG, ...this.config.retry };
    ...
    const tryRetry = async (urlIndex: number, statusKey: number, reason: string, response: Response | null = null) => {
      ...
      await new Promise((resolve: ProtocolValue) => setTimeout(resolve, waitMs));
      return true;
    };
    ...
    for (let urlIndex = 0; urlIndex < fallbackCount; urlIndex++) {
      ...
      const headers = this.buildHeaders(credentials, stream, url, model, transformedBody instanceof Uint8Array ? undefined : transformedBody);
