...
function concatBytes(arrays: readonly Uint8Array[]) {
  const total = arrays.reduce((n: number, a: Uint8Array) => n + a.length, 0);
...
interface WindsurfMessage { role: string; content: string; toolCallId?: string }

function buildChatMessage(msg: WindsurfMessage) {
...
export function buildGetChatMessageRequest(apiKey: string, model: string, messages: WindsurfMessage[]) {
...
type CompletionChunk = { kind: "content"; text: string } | { kind: "done"; promptTokens: number; completionTokens: number } | { kind: "error"; message: string } | { kind: "unknown" };

export function decodeCompletionChunk(buf: Uint8Array): CompletionChunk {
  let offset = 0;
  while (offset < buf.length) {
    let tag;
    [tag, offset] = readVarint(buf, offset);
...
function openAIMessagesToWs(messages: ProtocolRecord[]) {
  const out: WindsurfMessage[] = [];
...
export class WindsurfExecutor extends BaseExecutor {
  ...
  transformRequest(_model: string, body: ProtocolRecord) {
    return body;
  }

  async execute({ model, body, stream, credentials, signal, log, upstreamExtraHeaders, proxyOptions = null }: ExecutorRequest & { upstreamExtraHeaders?: Record<string, string> }): Promise<ExecutorResult> {
    ...
    const sseStream = new ReadableStream<Uint8Array>({
      async start(controller: ReadableStreamDefaultController<Uint8Array>) {
        ...
        try {
          ...
          const finishPayload: ProtocolRecord = {
            id: responseId, object: "chat.completion.chunk", created, model,
